...
def _EndRecData(fpin):
  ...
  try:
    ...
    return None
  data = fpin.read()
  if (len(data) == sizeEndCentDir and data[0:4] == stringEndArchive and
      data[-2:] == b'\000\000'):
...
class ZipInfo(object):
  ...
  def __init__(self, filename, orig_filename, date_time=(1980, 1, 1, 0, 0, 0)):
    self.orig_filename = orig_filename  # Original file name in archive
...
class ZipFile:
  ...
  def __init__(self,
    ...
    """
    if mode != 'r':
      raise NotImplementedError("ZipFile can only read")
  ...
  def _RealGetContents(self):
    ...
    while total < size_cd:
      ...
      try:
        ...
        orig_filename = orig_filename.decode("utf-8")
      except UnicodeDecodeError:
        raise
        #orig_filename = f"{filename}"
        #filename = f"{filename}"
      # Create ZipInfo instance to store file information
      x = ZipInfo(filename, orig_filename)
  ...
  def _extract_member(self, member, targetpath, pwd):
    ...
    invalid_path_parts = (b'', os.path.curdir, os.path.pardir)
