...
#define xorstr_always(str) ::jm::xor_string<XORSTR_STR(str), __LINE__ * 100 + __COUNTER__ + __TIME__[7] * 10 + __TIME__[8]>()
#ifndef XORSTR_DISABLE
#define xorstr(str) ::jm::xor_string<XORSTR_STR(str),  __LINE__ * 100 + __COUNTER__ + __TIME__[7] * 10 + __TIME__[8]>()
...
template<std::size_t S, std::size_t X>
constexpr std::uint64_t key8()
{
  constexpr auto first_part  = key4<(2166136261 + S + X) ^ COMPILER_SEED>();
...
struct xor_string {
  ...
  XORSTR_FORCEINLINE void _crypt() noexcept
  {
    if constexpr(detail::buffer_size<T>() > N) {
      ...
      if constexpr((detail::buffer_size<T>() - N) >= 4) {
        ...
        keys[0] = detail::key8<N + 0, X>();
        keys[1] = detail::key8<N + 1, X>();
        keys[2] = detail::key8<N + 2, X>();
        keys[3] = detail::key8<N + 3, X>();
  ...
  XORSTR_FORCEINLINE constexpr static std::uint64_t _at()
  {
    ...
    constexpr std::uint64_t val = detail::string_storage<T, X>{}.storage[N];
    return val;
  }
