...
gameState.GameStateManager = class GameStateManager {
  ...
  restartConnection() {
    ...
    this.connectionStartTick = this.currentState.tick
  }
  ...
  processNewChanges(changes) {
    ...
    const minAcceptableTickNumber = Math.max(
          maxPossibleTickNumber - gameState.MAX_BUFFERED_TICKS, 1)

    while (changes.length > 0) {
      // Verify whether the tick doesn't exceed possible tick count.
      const expectedTick = this.currentState.tick + 1
      if (expectedTick < minAcceptableTickNumber) {
        // TODO: Debug stuff.
        console.error("State arrived too late (lagged out).")
        return false
      }
      ...
      if (!gameState.GameState.compare(proposedState, simulatedState)) {
        // TODO: Debug stuff.
        console.error("State mismatch. Showing proposed, then simulated:")
        console.table(proposedState.state)
        console.table(simulatedState.state)
        return false
      }
