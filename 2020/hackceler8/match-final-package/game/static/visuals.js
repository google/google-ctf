...
visuals.Visuals = class Visuals {
  ...
  startEndSequence() {
    const startTime = globals.state.state.startTime
    const teamName = globals.state.state.teamName
    const elEndVideo = document.createElement("VIDEO")
    elEndVideo.preload = "auto"
    elEndVideo.src = "/end_crf23vsdb.mp4"
    elEndVideo.id = "end-video"
    elEndVideo.loop = true
    elEndVideo.muted = true
    elEndVideo.autoplay = true
    elEndVideo.style.display = "block"
    elEndVideo.style.opacity = 1

    const elText = document.createElement("DIV")
    elText.id = "end-panel-info"

    const elTime = document.createElement("DIV")
    elTime.id = "end-panel-time"

    const elLogo = document.createElement("IMG")
    elLogo.id = "end-panel-logo"
    elLogo.src = `/${teamName}_Logo.png`

    this.elEndPanel.innerHTML = ""
    this.elEndPanel.appendChild(elEndVideo)
    this.elEndPanel.appendChild(elText)
    this.elEndPanel.appendChild(elTime)
    this.elEndPanel.appendChild(elLogo)

    const matchDataRequest = new Request("/status")
    fetch(matchDataRequest)
      .then(res => res.json())
      .then(data => {
        let escapeText = "??:??"
        let infoText = "Match Completed!\n\n"

        data.forEach(ch => {
          const t = ch[1] - startTime
        ...
        elText.textContent = infoText
        elTime.textContent = escapeText
      })


    this.elEndPanel.style.display = "block"
    this.elEndPanel.style.opacity = 0
    let opacity = 0
    ...
    window.setTimeout(() => {
      ...
      const handle = window.setInterval(() => {
        ...
        if (opacity >= 1) {
          ...
          this.elEndPanel.style.opacity = 1
        } else {
          this.elEndPanel.style.opacity = opacity
        }
      }, 50)
    }, 3000)
  }
