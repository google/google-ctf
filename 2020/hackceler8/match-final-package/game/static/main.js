...
main.hideLoginForm = () => {
  const e = document.getElementById("login-form-container")
  if (e) {
    e.style.display = "none"
  }
}
...
main.showLoginForm = () => {
  ...
  const elVideo = document.getElementById("start-video")
  if (elVideo && elVideo.readyState === 4) {
    elVideo.pause()
...
main.wsOnMessage = (e) => {
  ...
  if (data.type === "startState") {
    ...
    if (globals.showLoginAnimation) {
      if (globals.state.state.tick !== 0) {
        main.hideLogin()
      } else {
        const elVideo = document.getElementById("start-video")
        if (elVideo && elVideo.readyState === 4) {
  ...
  if (data.type === "plzAuth") {
    ...
    if (fastToken) {
      ...
      globals.showLoginAnimation = false
      return
    }

    // Fallback to re-login.
    main.showLogin()
    globals.showLoginAnimation = true
  }
  ...
  if (data.type === "authOK") {
    ...
    if (!globals.showLoginAnimation) {
      ...
      // Otherwise the animation will be played when state is received as we
      // need to check if this isn't a reconnect.
    }
  }
