...
mapUtils.MapFrameset = class MapFrameset {
  ...
  getFrame(state, frameIndex) {
    if (!this.states.hasOwnProperty(state)) {
      if (this.defaultFrame === null) {
        console.error("returning null from getFrame (1). state", state, "frameIndex", frameIndex)
      }
      return this.defaultFrame
    }
    ...
    if (frameIndex >= stateFrames.length) {
      if (this.defaultFrame === null) {
        console.error("returning null from getFrame (2). state", state, "frameIndex", frameIndex)
      }
      return this.defaultFrame
    }

    if (stateFrames[frameIndex] === null) {
      console.error("returning null from getFrame (3). state", state, "frameIndex", frameIndex)
    }
  ...
  processTileset() {
    this.tileset.tiles.forEach(tile => {
      if (tile.collisions !== undefined) {
        // Fill out any properties for specific collision rects
        tile.collisions.forEach(collision => {
          if (collision.hasOwnProperty("properties") && collision.properties.forEach !== undefined) {
            var props = collision.properties
            collision.properties = this.defaultProperties()
            props.forEach(property => {
              collision.properties[property.name] = property.value
            })
          } else if (!collision.hasOwnProperty("properties")){
            collision.properties = this.defaultProperties()
          }
        })
      }
