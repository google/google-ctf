...
entities.classes.Player = class Player {
  static init(state, data) {
    ...
    this.aboveLastSolidGround = false  // Whether the player has jumped above their last solid ground point
    this.lastSolidGroundTick = 0 // The tick where this.lastSolidGround was set
  ...
  static tick(state, input) {
    ...
    const hasCoyoteTime = (
      this.y >= this.lastSolidGround[1]
      && !this.aboveLastSolidGround
      && (state.state.tick - this.lastSolidGroundTick) * gameState.SEC_PER_TICK <= this.COYOTE_TIME
      )
    if (!this.canJump && (this.solidGround || hasCoyoteTime) && !(
      "up" in input)) {
      this.canJump = true
    }
    ...
    const jumpStart = ("up" in input) && (this.jumpProgress == -1) && this.canJump
    if (jumpStart) {
    ...
    if (this.solidGround) {
      this.aboveLastSolidGround = false
    } else if (this.y < this.lastSolidGround[1]) {
      this.aboveLastSolidGround = true
    }
    ...
    if (this.solidGround && !this.dead) {
      ...
      this.lastSolidGroundTick = state.state.tick
    }
    ...
    if ("flag" in input && !challengeState.solved) {
      ...
      if (state.backendObjects) {
        ...
        if (flagHash === correctHash) {
          ...
          state.backendObjects.playersDB.saveMeta(state.backendObjects.username, state.state)
        }
      } else {
        // Browser.
        const flagUI = globals.game.getFlagConsoleUIObject(this.challengeID)
...
entities.classes.Savepoint = class Savepoint {
  ...
  static tick(state, input) {
    ...
    if (this.showing && this.interacting && "saveComplete" in input && !state.backendObjects) {
      // Happens once we recieve saveComplete - display that.
      const saverUI = globals.game.getGameSaverUIObject()
    ...
    if ("save" in input && this.showing && this.saveLatch == false) {
      ...
      if (state.backendObjects) {
        ...
        this.showing = false  // Ensure .showing is false in the saved game, so that menu can be opened after loading
        state.backendObjects.playersDB.saveSavegame(state.backendObjects.username, input.save, state.state)
        this.showing = true

        // Notify the client that the save is complete.
        state.backendObjects.gameService.sendMessage(state.backendObjects.username, {
          "type": "saveComplete"
        })
      } else {
        // Browser.
        const saverUI = globals.game.getGameSaverUIObject()
        saverUI.setStatus("saving")
      }
      this.saveLatch = true
    } else {
      this.saveLatch = false
...
entities.classes.Spawner = class Spawner {
  static init(state, data) {
  ...
  static handleLogic(state, sender, value) {
    ...
    this.endTick = (state.state.tick + this.duration / gameState.MS_PER_TICK * 1000)|0
    this.pendingValueX = utils.serverRandom(this.endTick)
    this.pendingValueY = utils.serverRandom(this.endTick)

    this.frameState = "blinky"
    entities.animResetFrameInfo(this, state)
  }

  static tick(state, inputs, consumedRngIds) {

    if (state.state.tick === this.endTick) {
      delete this.endTick
      this.frameState = "default"
      entities.animResetFrameInfo(this, state)
      ...
      if (new_entity !== null) {
        var offsetX = utils.finalServerRandom(state.state.tick, this.pendingValueX, state.backendObjects, consumedRngIds)
        if (offsetX === null) {
          offsetX = 0
        }
        var offsetY = utils.finalServerRandom(state.state.tick, this.pendingValueY, state.backendObjects, consumedRngIds)
        if (offsetY === null) {
          offsetY = 0
        }
        new_entity.x = this.x + 8 + (offsetX % 32)
...
entities.classes.Rng = class Rng {
  ...
  static handleLogic(state, sender, value) {
    ...
    this.endTick = (state.state.tick + this.randomDuration / gameState.MS_PER_TICK * 1000)|0
    this.pendingValue = utils.serverRandom(this.endTick)
  }

  static tick(state, inputs, consumedRngIds) {
    if (state.state.tick === this.endTick) {

      var randval = utils.finalServerRandom(state.state.tick, this.pendingValue, state.backendObjects, consumedRngIds) % 2
      if (randval === null) {
        randval = 0
      }
...
entities.classes.FloatingPlatform = class FloatingPlatform {
  static init(state, data) {
    ...
    if (data.properties.input) {
      this.defaultFrameState = this.frameState
      entities.subscribeLogic(this, state, data.properties.input)
    }

    if (data.properties.powered !== undefined) {
      this.powered = data.properties.powered
      if (!this.powered) {
        this.frameState = "off"
      }
    } else {
    ...
    if (data.properties.attachedObject) {
      this.attachedObject = data.properties.attachedObject
    }
  ...
  static handleLogic(state, sender, value) {
    this.powered = value
    if (value) {
      this.frameState = this.defaultFrameState
    } else {
      this.frameState = "off"
    }
  ...
  static tick(state) {
    ...
    if (this.attachedObject) {
      const attachedObject = state.state.entities[this.attachedObject]
      if (attachedObject !== undefined) {
        attachedObject.x += (this.x - oldX)
        attachedObject.y += (this.y - oldY)
      }
    }
  }
}

entities.animationEngines.ItemBarrier = class ItemBarrier {
  static init(state) {
    this.frameState = "open3"
    entities.animResetFrameInfo(this, state)
  }
...
entities.classes.ItemBarrier = class ItemBarrier {
  static init(state, data) {
    this.collision = false;
    entities.animationEngines.ItemBarrier.init.call(this, state)
  }

  static tick(state) {
    const player = state.state.entities.player
    this.collision = !!player.heldEntityId
    entities.animationEngines.ItemBarrier.tick.call(this, state)
  }
}
...
entities.subscribeLogic = (subscribingEntity, state, inputEntityId) => {
  const publisherEntity = state.state.entities[inputEntityId]

  if (publisherEntity === undefined) {
    console.error("Could not subscribe to nonexistent entity", inputEntityId, state.state.entities)
    return
  }
  if (publisherEntity.subscribers === undefined) {
    publisherEntity.subscribers = []
  }
  publisherEntity.subscribers.push(subscribingEntity.id)
}

// Publish a logic-gate update to subscribers.
entities.updateLogic = (sendingEntity, state) => {
  const subscribers = sendingEntity.subscribers
  if (subscribers === undefined) {
    return
  }
  subscribers.map(subscriberId => {
    const subscriber = state.state.entities[subscriberId]
    if (subscriber === undefined) {
      return  // Entity no longer exists
    }
    const typ = entities.classes[subscriber.type]
    if (typ === undefined) {
      console.error("Attempting to send logic update to entity without corresponding class: ", subscriber)
      return
    }
    if (!typ.hasOwnProperty("handleLogic")) {
      console.error("Attempting to send logic update to entity without handleLogic function: ", subscriber)
      return
    }
    typ.handleLogic.call(subscriber, state, sendingEntity.id, sendingEntity.logicOutput)

  })
}

entities.classes.EntryAnimation = class EntryAnimation {
  static init(state, data) {
    this.collision = false;
    this.frameState = "blinky"
    entities.animResetFrameInfo(this, state)
  ...
  static tick(state) {
    if (!this.endOfFrames) {
      this.endOfFrames = entities.animTickFrame(this, state)
      if (this.endOfFrames) {
        this.frameState = "off"
        entities.animResetFrameInfo(this, state)
      }
