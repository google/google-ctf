...
class LocalPlayersDatabase extends PlayersDatabaseBase {
  ...
  _ensureInitSavegames(username) {
    ...
    for (const fn of filenames) {
      ...
      if (game.is_meta) {
        this.meta[username] = game
        console.log("Loaded meta", game)
        continue
      }
      if (game.is_backup === true && game.name !== this.preloadedBackupName) {
        continue
      }
      this.savegames[username][game.name] = utils.simpleDeepCopy(game)
  ...
  */
  ...
  _saveSavegameImpl(username, savename, state, is_backup) {
    ...
    const savegameEntry = {"name": savename, "timestamp": timestamp, "state": utils.simpleDeepCopy(state), "is_backup": is_backup}
    ...
    var savename_f = savename_f.replaceAll("meta", "no")
  ...
  saveMeta(username, state) {
    this._ensureInitSavegames(username)
    const dir = this._savegameDir(username)
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir, 0o744);
    }
    const filename = dir + "/meta.json"
    const meta = utils.simpleDeepCopy(state.meta)
    meta.is_meta = true
    fs.writeFileSync(
        filename,
        JSON.stringify(meta));
    this.meta[username] = meta
  }

  loadMeta(username, state) {
    this._ensureInitSavegames(username)
