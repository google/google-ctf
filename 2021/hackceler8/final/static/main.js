...
main.showSavegames = (savegameMeta) => {
  const e = document.getElementById("scene-load")
  const slist = document.getElementById("savegame-list")
  slist.innerHTML = ""
  const currentTime = Math.floor(+new Date() / 1000)

  const form = document.getElementById("loadgame-form")
  if (savegameMeta.length == 0) {
    form.style.display = "none"
  } else if (form.style.hasOwnProperty("display")) {
    form.style.removeProperty("display")
  }

  var had_backup = false

  savegameMeta.forEach(savegame => {
    var deltaSecs = currentTime - savegame.timestamp
    var deltaMins = Math.floor(deltaSecs / 60)
    deltaSecs = deltaSecs % 60

    const template = document.getElementById("savegame-entry")
    const fragment = template.content.cloneNode(true)
    const a = fragment.querySelector("a")
    a.innerText = savegame.name + " | " + deltaMins.toString() + "m" + deltaSecs.toString() + "s ago"
    if (savegame.is_backup) {
      fragment.querySelector('.savegame-entry-box').style = "background-color: grey; color: blue; font-weight: bold;"
      slist.prepend(fragment)
      had_backup = true
    } else {
      slist.appendChild(fragment)
    }

    const backup_alert = document.getElementById("backup-alert")
    if (!had_backup) {
      backup_alert.style.display = "none"
    } else if (backup_alert.style.hasOwnProperty("display")) {
      backup_alert.style.removeProperty("display")
    }
...
main.wsOnMessage = (e) => {
  ...
  if (data.type === "savegames" && !observer) {
    const continueButton = document.getElementById("continuegame")
    if (data.continueAble) {
      if (continueButton.style.hasOwnProperty("display")) {
        continueButton.style.removeProperty("display")
      }
    } else {
      continueButton.style.display = "none"
    }
    main.showSavegames(data.savegameMeta)
  }
  ...
  if (data.type === "saveComplete") {
    globals.game.processSaveComplete()
  }
  ...
  if (data.type === "stateError") {
    main.hasStateError = true
    console.error(data)
    if (!main.hasDelayedStateError) {
      if (data.message) {
        main.showError(data.message)
      } else {
        main.showError("Unknown server error.")
      }
    }
    if (data.delayed_message) {
      setTimeout( function() {
        if (main.hasStateError === true) {
          main.hasDelayedStateError = true
          delete main.hasStateError
          main.showError(data.delayed_message)
          globals.game.stop()
        }
      }, 3000);
    }
    if (data.state !== undefined) {
      console.log("Resetting state due to desync.")
      globals.state = gameState.GameState.fromStateDict(data.state, globals.map)
      globals.game.start()
    }
  }
  if (data.type === "stateErrorClear") {
    console.log(data)
    if (data.message) {
      main.showError(data.message)
      main.hasStateError = false
      setTimeout( function() {
        if (main.hasStateError === false && main.hasDelayedStateError === undefined) {
          delete main.hasStateError
          main.showError("")
        }
      }, 4000);
  ...
  if (data.type === "randomNumber") {
    ...
    for(var k in Object.keys(main.pendingServerRandoms[data.tick])) {
      if (main.pendingServerRandoms[data.tick][k].id === data.id) {
        main.pendingServerRandoms[data.tick][k].value = data.randResult
        foundEntry = true
        break
      }
    }
    if (!foundEntry) {
      console.error("Failed to find random number entry for:", data)
      console.log("Random number entries:", main.pendingServerRandoms)
    }
  }
}
...
main.main = () => {
  ...
  if (!observer) {
    const f = document.getElementById("newgame")
    f.addEventListener("click", (ev) => {
      main.handleNewGame()
      ev.preventDefault()
    })
    const g = document.getElementById("continuegame")
...
main.pendingServerRandoms = {}
main.nextServerRandomId = 0
main.consumedRngIdsList = []

main.serverRandom = (target_tick) => {
  if (main.pendingServerRandoms[target_tick] === undefined) {
    main.pendingServerRandoms[target_tick] = []
  }

  const sentinel = ["__SENTINEL__"]

  const id = main.nextServerRandomId++
  main.pendingServerRandoms[target_tick].push({"sentinel": sentinel, "value": null, "id": id})
  main.wsSend({
...
main.tickReady = (tick) => {
  if (main.pendingServerRandoms[tick] === undefined) {
    return true
  }

  for (var p in main.pendingServerRandoms[tick]) {
    if (main.pendingServerRandoms[tick][p].value === null) {
      return false
    }
  }
  return true
...
main.finalServerRandom = (target_tick, sentinel) => {
  if (main.pendingServerRandoms[target_tick] === undefined) {
    console.log("finalServerRandom asked for target_tick=" + target_tick.toString() + ", which has no values. This usually indicates that the client was disconnected while RNG was pending.")
    return null
  }
  for (var k in main.pendingServerRandoms[target_tick]) {
    const p = main.pendingServerRandoms[target_tick][k]
    if (p.sentinel === sentinel) {
      if (p.value === null) {
        console.error("finalServerRandom Somehow got missing random value on target_tick=" + target_tick.toString())
        return null
      }
      main.consumedRngIdsList.push(p.id)
      return p.value
    }
  }
  return null
...
main.consumedRngIds = () => {
  const ret = main.consumedRngIdsList
  main.consumedRngIdsList = []
  return ret
}
