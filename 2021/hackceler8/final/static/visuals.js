...
visuals.Visuals = class Visuals {
  ...
  render() {
    ...
    this.checkCutscene(playerEntity.x, playerEntity.y)
    ...
    let solvedCount = 0
    for (let k in s.meta.challenges) {
      solvedCount += s.meta.challenges[k].solved
    }
    ...
    if (this.lastSolvedCount != solvedCount) {
      ...
      var i = 0
      for (let k in s.meta.challenges) {
        const chal_idx = parseInt(k.substr(-1))
        if (s.meta.challenges[k].solved) {
          this.elFlags[chal_idx].style.display = "block"
        } else {
          this.elFlags[chal_idx].style.display = "none"
        }
      }
    }
  ...
  renderEntity(e) {
    ...
    if (e.flipImage) {
      ...
      ctx.drawImage(
          img,
          tile.x|0, tile.y|0, tile.tileW, tile.tileH,
          (-e.x - tile.tileW + xOffset)|0, (e.y - yOffset)|0, tile.tileW, tile.tileH
      )
      ctx.restore()
    } else {
      ctx.drawImage(
          img,
          tile.x|0, tile.y|0, tile.tileW, tile.tileH,
          (e.x - xOffset)|0, (e.y - yOffset)|0, tile.tileW, tile.tileH
      )
    }

    this.playerCanvasClearRects.push([(e.x - xOffset)|0, (e.y - yOffset)|0, tile.tileW, tile.tileH])
    return true
  }


  checkCutscene(playerX, playerY) {
    if (this.currentCutscene &&
        this.isPointInBoundingBox(playerX, playerY, this.currentCutscene.bb)) {
      // Player is still in 'active' cutscene, inputs should be in the cutsceneQueue.
      return
    }
    let newCutscene = null

    const cutsceneCount = this.cutscenes.length
    for (let i = 0; i < cutsceneCount; i++) {
      const cutscene = this.cutscenes[i]
      if (this.isPointInBoundingBox(playerX, playerY, cutscene.bb)) {
        newCutscene = cutscene
        break
      }
    }

    if (this.currentCutscene === null && newCutscene === null) {
      // Player is still outside of any Cutscene.
      return
    }

    if (newCutscene === null) {
      // Player left defined cutscene
      this.currentCutscene = null
      return
    }

    // Player is in an actual Cutscene.
    this.currentCutscene = newCutscene

    // Set inputs, decode JSON
    globals.game.cutsceneInputQueue = JSON.parse(newCutscene.inputs)

    // Disable HUD as soon as we enter the cutscene, this gets re-enabled as
    // soon as all the inputs have been processed in `game/static/game.js`.
    this.elMainHud.style.visibility = "hidden"
    if (this.currentCutscene.message) {
      main.showError(this.currentCutscene.message)
      setTimeout(function(){
        main.hideError()
      }, this.currentCutscene.messageDuration * 1000)
    }
  ...
  updateAreaIfNeeded(playerX, playerY) {
    ...
    this.elParallax.forEach(c => {
      c.style.display = "none"
    })
    if (newArea.parallaxLayers !== undefined) {
      newArea.parallaxLayers.forEach(s => {
        document.getElementById(s).style.removeProperty("display")
      })
    } else {
      this.elParallax.forEach(c => {
        if (c.default_show) {
  ...
  initializeGameAreas() {
    ...
    areas.forEach(area => {
      var parallax_layers = area.properties.parallax_layers
      if (parallax_layers !== undefined) {
        parallax_layers = parallax_layers.split("\n")
      }
      this.gameAreas.push({
  ...
  initializeCutscenes() {
    const cutscenes = this.map.layers.metadata.objectsByType.cutscene
    if (!cutscenes) {
      return
    }
    ...
    cutscenes.forEach(cutscene => {
      this.cutscenes.push({
        bb: [ cutscene.x, cutscene.y, cutscene.x + cutscene.width, cutscene.y + cutscene.height ],
        name: cutscene.name,
        inputs: cutscene.properties.inputs,
        message: cutscene.properties.message,
        messageDuration: cutscene.properties.messageDuration
      })
    })
  }
  ...
  initializeParallax() {
    this.initializeLayerType("parallax", (name, idx, layer) => {
      ...
      c.default_show = layer.rawProperties.default_show
      if (c.default_show === false) {
        c.style.display = "none"
      }
    })
  ...
  initializeCanvases() {
    ...
    this.elEntities = this.createCanvas(RES_W, RES_H, 300)
    this.elContext = this.elEntities.getContext("2d")

    this.initializeLayerType("fg", (name, idx, layer) => {
      const c = this.createCanvas(this.map.canvasW, this.map.canvasH, idx * 200 + 200)
      this.drawTiles(c, layer)
      this.elFore.push(c)
    })
