...
gameState.GameState = class GameState {
  ...
  initialize(pc) {
    ...
    var odd_found = -1
    var even_found = -1
    console.log("Starting seed search")
    while (true) {
      odd_found = -1
      even_found = -1
      seed = makerandom(5)
      var randState = utils.initDeterministicRandomState(seed)

      for (var i = 0; i < 512; ++i) {
        utils.deterministicRandom(randState) % 2

        var tmpRandom = [randState[0]]
        var was_all_1 = true
        for (var j = 0; j < 6; ++j) {
          var val = utils.deterministicRandom(tmpRandom) % 2
          if (val === 0) {
            was_all_1 = false
          }
        }
        if (was_all_1) {
          console.log("Random found", seed, i)
          if (i % 1) {
            odd_found = i;
          } else {
            even_found = i;
          }
          if (odd_found !== -1 && even_found !== -1) {
            console.log("Random found", seed, odd_found, even_found)
            return
          }
        }
      }
  ...
  calculatePotentialMove(p, v, frame, excludedEntities) {
    ...
    const [finalPosition, solidGround, jumpzone] = this.calculatePotentialMoveWorker(
        p, v, frame,
    ...
    return [finalPosition, solidGround, jumpzone, collidingSoftEntities]
  }
  ...
  calculatePotentialMoveWorker(
    ...
    // movement).
    const [oldCollision, oldSolidGround, oldJumpzone] = this.checkMovementCollision(
    ...
    if (oldCollision) {
      ...
      for (let j = 1; j <= 20; j++) {
        for (let i = 0; i < 8; i++) {
          ...
          let [candidateCollision, candidateSolidGround, candidateJumpzone] = this.checkMovementCollision(
            mapRects, entityRects
          )

          if (!candidateCollision) {
            return [candidateP, candidateSolidGround, candidateJumpzone]
          }
        }
      }
      ...
      return [p, oldSolidGround, oldJumpzone]
    }
    ...
    const [resultCollision, resultSolidGround, resultJumpzone] = this.checkMovementCollision(
        mapRects, rectsNewPosition
    )

    if (!resultCollision) {
      return [maxMove, resultSolidGround, resultJumpzone]
    }
    ...
    const [idx, solidGroundAtSafePosition, jumpzoneAtSafePosition] = this.iterateCollision(
    ...
    const [idxX, solidGroundAtIdxX, jumpzoneAtIdxX] = this.iterateCollision(
        mapRects, intPositionsX, frame
    )

    const [idxY, solidGroundAtIdxY, jumpzoneAtIdxY] = this.iterateCollision(
    ...
    if (idxX === null && idxY === null) {
      if (solidGroundAtSafePosition === null) {
        ...
        return [safePosition, resultSolidGround, resultJumpzone]
      } else {
        return [safePosition, solidGroundAtSafePosition, jumpzoneAtSafePosition]
      }
    }

    if (idxX === null || (idxY !== null && idxY >= idxX)) {
      return [intPositionsY[idxY], solidGroundAtIdxY, jumpzoneAtIdxY]
    }

    return [intPositionsX[idxX], solidGroundAtIdxX, jumpzoneAtIdxX]
  }
  ...
  iterateCollision(mapRects, positions, frame) {
    ...
    let lastGoodJumpzone = null

    for (let i = 0; i < positions.length; i++) {
      const entityRects = this.getCollisionsRectsForTile(frame, positions[i])
      let [resultCollision, resultSolidGround, resultJumpzone] = this.checkMovementCollision(
          mapRects, entityRects
      )

      if (resultCollision) {
        return [lastGood, lastGoodSolidGround, lastGoodJumpzone]
      }

      lastGood = i
      lastGoodSolidGround = resultSolidGround
      lastGoodJumpzone = resultJumpzone
    }
  ...
  checkMovementCollision(mapRects, entityRects) {
    ...
    return [collisionTypes.has("solid"), collisionTypes.has("gravity"), collisionTypes.has("jumpzone")]
  }
