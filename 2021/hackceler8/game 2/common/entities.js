...
entities.classes.Lightbridge = class Lightbridge {
  static init(state, data) {
    this.collision = true
    if (data.properties.frame_state === undefined) {
      this.frameState = "purple"
    } else {
      this.frameState = data.properties.frame_state
    }
    this.backupFrameState = this.frameState
    entities.animResetFrameInfo(this, state)

    this.fallthroughSet = data.properties.fallthroughSet

    if (state.state.fallthroughCountdowns === undefined) {
      state.state.fallthroughCountdowns = {}
    }
    if (state.state.fallthroughCountdowns[this.fallthroughSet] == undefined) {
      state.state.fallthroughCountdowns[this.fallthroughSet] = 0
    }
  }

  static interact(state, input, types) {
    if (state.state.fallthroughCountdowns[this.fallthroughSet] > 0) {
      state.state.fallthroughCountdowns[this.fallthroughSet] = 3
    }
  }

  static tick(state) {
    const player = state.state.entities.player
    if (player.canJump && Math.abs(player.moveV) > 0.01) {
      state.state.fallthroughCountdowns[this.fallthroughSet] = 3
    }
    ...
    if (state.state.fallthroughCountdowns[this.fallthroughSet] === 0) {
      this.collision = true
      this.frameState = this.backupFrameState
      if (this.collision != oldCollision) {
        entities.animResetFrameInfo(this, state)
      }
    } else {
      state.state.fallthroughCountdowns[this.fallthroughSet]--;
      this.collision = false
      this.frameState = "grey"
      if (this.collision != oldCollision) {
        entities.animResetFrameInfo(this, state)
      }
    }
