...
static long cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
  ...
  switch (cmd) {
    ...
    case 2:
      mutex_lock(&mutex);
      rc = device_init(pci_get_drvdata(g_pci_dev));
      mutex_unlock(&mutex);
      return rc;
    case 3:
      mutex_lock(&mutex);
      rc = device_reset(pci_get_drvdata(g_pci_dev));
      mutex_unlock(&mutex);
      return rc;
    ...
    case 1337:
      ...
      rc = pwn_device((struct private_data *)pci_get_drvdata(g_pci_dev), arg);
      mutex_unlock(&mutex);
      return rc;
...
static ssize_t cdev_write(struct file *file, const char *buf, size_t length,
  ...
  printk(KERN_INFO "Sending %d bytes\n", actual_length);
