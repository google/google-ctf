...
RUN apt update -o Acquire::Check-Valid-Until=false && apt install -yq --no-install-recommends \
	build-essential \
	$(apt-cache depends chromium | grep Depends | awk '{print $2}' | grep -v chromium) \
	$(apt-cache depends chromium-common | grep Depends | awk '{print $2}') \
	python3 \
	kmod
...
COPY chromium/ /usr/lib/chromium/
COPY ctf.ko /
COPY init /
COPY run_chromium.py /

# Fix the permissions for the files we're copying in
RUN chown -R 0:0 /usr/lib/chromium /run_chromium.py /ctf.ko /init && \
	chmod -R a+r /usr/lib/chromium && \
	find /usr/lib/chromium -type d -exec chmod 755 {} + && \
	chmod a+x /usr/lib/chromium/chrome && \
	chmod a+r /run_chromium.py && \
	chmod +x /init
