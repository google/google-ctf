...
def addl_draw_info(b):
    if b.ticks > 270 and b.game.player.immobilized:
        return [gfx.circle_outline(b.game.player.x, b.game.player.y, 50, (180,0,0,255), border_width=5)]
    return []
...
def fight(b):
    ...
    if b.game.player.dead:
        b.game.player.immobilized = False
    ...
    if b.state.timer <= 0:
        ...
        if b.game.player.dead or b.state.slash_timer > 0:
            b.state.name = "idle"
        elif b.state.name == "teleport" or b.state.name == "freeze":
            b.state.name = "walk"
            ...
            while True:
                if len(b.state.next_states) == 0:
                    b.state.next_states = ["idle", "walk", "shoot", "freeze"] * 3
                    b.state.rnd.shuffle(b.state.next_states)
                b.state.name = b.state.next_states.pop()
                # No freeze action if the player is already immobilized.
                if not (b.state.name == "freeze" and b.game.player.immobilized):
                    break
        match b.state.name:
            case "idle":
                b.state.timer = 60
            ...
            case "freeze":
                b.state.timer = 180
                b.state.px = b.game.player.x
                b.state.py = b.game.player.y
                b.game.player.immobilized = True
    ...
    match b.state.name:
        ...
        case "teleport":
            if b.state.timer == 45:
                ...
                b.place_at(pos, b.y)
        case "freeze":
            if b.game.player.immobilized:
                pr = 1 - (b.state.timer / 180)
                b.game.player.place_at(
                    b.state.px + pr * (500 - b.state.px),
                    b.state.py + pr * (300 - b.state.py))
                b.game.player.reset_speed()

    if b.game.player.immobilized and b.state.name != "freeze":
        b.game.player.place_at(500, 300)
        b.game.player.reset_speed()
