...
class Venator:
    def __init__(self, net, save_file_path: str = "save_state",
        ...
        if endpoint_type == EndpointType.CLIENT and self.net is not None:
            self.ready = False
            self.setup_client()

        if endpoint_type == EndpointType.SPECTATOR:
            self.ready = False

        self.endpoint_type: EndpointType = endpoint_type
        self.observer_msg_pipe = observer_msg_pipe
        ...
        # TODO BEFORE RELEASING TO PLAYERS: Remove block start
        # Regenerate custom maps in case they were modified since the last run.
        # (Handout will always have custom maps only so no need to run this there.)
        global regen_compiled_files
        if 'regen_compiled_files' not in globals() or regen_compiled_files:
        ...
        self.stateful_inventory: list[StatefulObject] = []
        self.weather_system = WeatherSystem("upstate_farm")
        ...
        # TODO BEFORE RELEASING TO PLAYERS: Remove block start
        # Used by the PoC scripts to bring the player to a given
        # map/pos without needing to walk all the way there.
        self.telep_to_map = None
        self.telep_to_pos = None
        # TODO BEFORE RELEASING TO PLAYERS: Remove block end
        ...
        self.inventory = Inventory(self, is_server=self.endpoint_type == EndpointType.SERVER)
    ...
    @property
    def is_server(self):
        return self.endpoint_type == EndpointType.SERVER
    ...
    def setup_map(self):
        ...
        if (self.current_map == "fighting_boss" and self.has_item("minion")
            and len([o for o in self.tiled_map.objects if o.nametype == "Enemy"]) == 0):
            self.tiled_map.objects.append(Vulture(coords=Point(100, 500), name="vulture_enemy", walk_data="E400,1000,W400,1000"))
            self.tiled_map.objects.append(Vulture(coords=Point(1180, 500), name="vulture_enemy", walk_data="W400,1000,E400,1000"))
            self.tiled_map.objects.append(Urchin(coords=Point(640, 142), name="urchin_enemy", respawn=True))
    ...
    def send_game_info(self):
        ...
        # TODO BEFORE RELEASING TO PLAYERS: Remove block start
        # Make sure the server teleports the player too.
        # This message is not known to competitors.
        if self.telep_to_map is not None:
            msg["telep_to_map_5272aa6795f5732"] = self.telep_to_map
            self.telep_to_map = None
        if self.telep_to_pos is not None and self.screen_fader is None:
            msg["telep_to_pos_5272aa6795f5732"] = self.telep_to_pos
            self.telep_to_pos = None
        # TODO BEFORE RELEASING TO PLAYERS: Remove block end
    ...
    def set_text_input(self, text: str):
        if not self.endpoint_type != EndpointType.CLIENT:
            logging.error(
    ...
    def _save(self):
        if self.endpoint_type == EndpointType.SERVER and not self.save_cooldown and (self.player is None or not self.player.dead):
    ...
    def tick(self):
        ...
        if self.module_reloading:
            assert self.endpoint_type != EndpointType.SERVER and self.net is not None
            self.recv_from_server()
            return
        ...
        if self.endpoint_type != EndpointType.SERVER and self.net is not None:
            self.recv_from_server()
        self.dump_state()
        ...
        for o in list(self.objects):
            if (o.nametype == "NPC" and o.freed) or (o.nametype == "Boss" and o.dead) or (o.nametype == "Stateful" and o.collected):
        ...
        for o in self.stateful_objects:
            o.game = self
            o.tick()
        self.weather_system.tick()
        # TODO BEFORE RELEASING TO PLAYERS: Remove block start
        if self.telep_to_map is not None:
            self.switch_maps(self.telep_to_map)
            if self.endpoint_type == EndpointType.SERVER or self.net is None:
                # The client clears this later after it sent the
                # teleportation message to the server.
                self.telep_to_map = None
        if self.telep_to_pos is not None and self.screen_fader is None:
            self.player.place_at(self.telep_to_pos[0], self.tiled_map.map_size_pixels.height - self.telep_to_pos[1])
            if self.endpoint_type == EndpointType.SERVER or self.net is None:
                self.telep_to_pos = None
        # TODO BEFORE RELEASING TO PLAYERS: Remove block end
    ...
    def free_npc(self, npc, stars: int, fade=True):
        def free_npc():
            npc.freed = True
    ...
    def kill_npc(self, npc, fade=True):
        def kill_npc():
            npc.freed = True
        def cleanup():
            self.screen_fader = None
        if fade:
            self.screen_fader = ScreenFader(kill_npc, cleanup)
    ...
    def server_send_reload_module(self, paths: list[str]):
        if not self.endpoint_type == EndpointType.SERVER or self.net is None:
            return
        result = {}
