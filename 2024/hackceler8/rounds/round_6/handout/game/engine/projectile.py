...
class ProjectileSystem:
    ...
    def _update_active_projectiles(self):
        for p in self.active_projectiles.copy():
            is_weapon_projectile = p.weapon is not None
            if p.check_oob(self.game.player):
                self.active_projectiles.remove(p)
                continue
            if not is_weapon_projectile or self.projectile_range > 0:
                p.move(p.x_speed, p.y_speed)
                if is_weapon_projectile:
                    self.projectile_range -= 1
            if self._check_collision(p) or (is_weapon_projectile and self.projectile_range == 0):
                self.active_projectiles.remove(p)
                if self.projectile_range == 0:
                    range = [w.range for w in self.game.player.weapons if w.equipped]
                    if len(range) > 0:
                        self.projectile_range = range[0]
                continue
            match p.origin:
                case "player":
