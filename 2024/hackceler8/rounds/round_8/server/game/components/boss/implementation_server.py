...
class SimpleBullet(Bullet):
    def __init__(
        ...
        super().__init__(boss, x, y, speed, angle, damage, radius=20, color=color)
...
def addl_draw_info(b):
    info = []
    if hasattr(b.state, "second_form") and b.state.second_form and b.state.sfx_timer > 0:
        prog = 1 - b.state.sfx_timer / 30
        info.append(gfx.circle_outline(
            b.x, b.y, 50 + prog * 200,
            (0, 0, 255, int(255 * min(1, 2 - prog * 2))),
            5))
    # Hack to make invulnerable.
    if (hasattr(b.state, "second_form") and b.state.second_form
        and b.state.name == "teleport" and b.state.real_name != "teleport"):
        info.append(b.sprite.get_draw_info(b.x, b.y))
    return info
...
def fight(b):
    ...
    if b.state.second_form:
        # Hack to make invulnerable.
        b.state.name = b.state.real_name
    ...
    # Slow down bullets.
    for p in b.game.projectile_system.active_projectiles:
        if b.game.tics % 4 == 0:
            p.prev_x = p.x
            p.prev_y = p.y
        else:
            p.place_at(p.prev_x, p.prev_y)
        if b.state.second_form and p.y_speed == 0 and p.expand(20).collides(b):
            p.x_speed = -p.x_speed
            p.y_speed = 10
    ...
    if b.state.timer <= 0:
        ...
        match b.state.name:
            ...
            case "teleport":
                b.state.timer = 90
        if not b.state.second_form and b.health <= 250:
            b.state.second_form = True
            b.state.sfx_timer = 30
            b.sprite.set_texture("resources/villain/fuego2.h8t")
    ...
    match b.state.name:
        ...
        case "shoot":
            if b.state.timer % 30 == 0:
                shoot(b, SimpleBullet(
                    b, b.x, b.y+20, 3.5,
                    angle_to_player(b, b.x, b.y+40),
                    damage=20 if b.state.second_form else 10,
                    color=(0, 22, 255, 0) if b.state.second_form else (255, 233, 0, 0)
                ))
        case "teleport":
            if b.state.timer == 45:
    ...
    if b.state.slash_timer > 0:
        ...
        if (b.state.slash_timer < 52 and not b.state.player_slashed
            ...
            b.game.player.decrease_health(40 if b.state.second_form else 20, "fighting_boss")
            b.game.player.sprite.set_flashing(True)

    update_animation(b)

    if b.state.second_form:
        b.state.sfx_timer = max(0, b.state.sfx_timer - 1)
        # Hack to make invulnerable.
        b.state.real_name = b.state.name
        b.state.name = "teleport"
