...
import game.components.boss.implementation_server

MARGIN = 50
BOSS_TITLE = 'Fuego Mk.IV: Two-face'
MAX_HEALTH = 500
HEALTHBAR_LEN = 1240
...
class FightingBoss(Boss):
    ...
    def get_draw_info(self):
        ...
        info += game.components.boss.implementation_server.addl_draw_info(self)
    ...
    def _get_hitboxes(self):
        if self.state.slash_timer > 0:
            box = self.state.slashbox_left if self.sprite.flipped else self.state.slashbox_right
            yield gfx.lrtb_rectangle_outline(
                left=box.x1, right=box.x2, top=box.y2, bottom=box.y1,
                color=(255, 0, 255, 255))

        for bullet in self.bullets:
            if not bullet.intangible:
                yield gfx.rectangle_outline(
                    bullet.x,
                    bullet.y,
                    bullet.hitbox_w,
                    bullet.hitbox_w,
        ...
        yield gfx.lrtb_rectangle_outline(
            self.x1, self.x2, self.y2, self.y1,
            (255, 0, 0, 255),
        )
    # TODO BEFORE RELEASING TO PLAYERS: Remove block end
    ...
    def reload_module(self):
        if self.game.is_server:
            self.game.server_send_reload_module(['game.components.boss.implementation_server'])
        else:
    ...
    def fight(self):
        return game.components.boss.implementation_server.fight(self)
    ...
    def reset(self):
        ...
        self.sprite.set_texture("resources/villain/fuego.h8t")
