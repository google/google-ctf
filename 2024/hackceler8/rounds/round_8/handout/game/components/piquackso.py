...
from interpreter.bf import Interpreter
import logging

DIRS = [Keys.D, Keys.W, Keys.A, Keys.S]
...
class Piquackso(generics.GenericObject):
    ...
    def interpret(self):
        _i = Interpreter(self.kode.encode())
        _i.start()
        _cmds = _i.execute_until_end()
        logging.info(_cmds)
        for c in _cmds:
            try:
                for i in range(20):
                    self.commands.append(DIRS[c])
            except Exception as e:
                logging.critical(f"Nah that's just wrong: {e}")
