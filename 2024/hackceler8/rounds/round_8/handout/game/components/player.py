...
class Player(generics.GenericObject):
    ...
    def get_draw_info(self) -> gfx.IterableParams:
        if self.spike is None:
            self.spike = gfx.load_image("resources/objects/spike.png", 0, 0, 16, 16)
        extra = []
        if self.spikes != 0:
            for yy in range(4):
                ty = 16 * yy + (self.y // 16) * 16 - 8
                tx = (self.x1 // 16) * 16 if self.spikes == 1 else (self.x2 // 16) * 16
                extra.append(gfx.SpriteDrawParams(x=tx,y=ty,tex=self.spike))
        return extra + [super().get_draw_info()]
    ...
    def update_position(self):
        ...
        if self.y < -50 and self.last_ground_pos is not None:
            self.place_at(self.last_ground_pos.x, self.last_ground_pos.y)
            self.decrease_health(20, "bottomless_pit")
            self.sprite.set_flashing(True)
