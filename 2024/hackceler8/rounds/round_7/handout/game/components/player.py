...
class Player(generics.GenericObject):
    ...
    def tick(self, pressed_keys, newly_pressed_keys):
        ...
        if not self.dead and self.game.tics % 360 == 0 and self.game.has_item("poison"):
            self.decrease_health(1, "poison")
    ...
    def update_movement(self, pressed_keys, newly_pressed_keys):
        ...
        if not self.scroller_mode:
            if not self.in_the_air and self.is_base_player:
                self.game.player_base_pos = [self.x, self.y]
