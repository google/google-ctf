...
class Venator:
    ...
    @property
    def should_invoke_server_dialogue(self):
        # Helper function to decide when to invoke the server dialogue functions vs the client one.
        # Only invoke them if we're the server or the client running in standalone mode.
        return self.endpoint_type == EndpointType.SERVER or (self.endpoint_type == EndpointType.CLIENT and self.net is None)
    ...
    def setup_map(self):
        ...
        for o in self.tiled_map.objects:
            ...
            if o.nametype == "Player":
                ...
                if (
                        self.current_map == "base"
                        and self.player_base_pos is not None
                ):
                    o.place_at(self.player_base_pos[0], self.player_base_pos[1])
                    o.in_the_air = False
