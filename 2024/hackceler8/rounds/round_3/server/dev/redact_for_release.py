...
files_to_rm = [
    ...
    "game/components/boss/fake-game",
...
def redact_file(path):
  is_fake_boss_file = path.endswith("dialogue_boss.py") and ("/boss/fake-game/" in os.path.abspath(path))
  ...
  for l in content.copy():
    if is_fake_boss_file:
      # In the boss file the players leak we show the full unredacted dialogue code.
      if ("TODO BEFORE RELEASING TO PLAYERS: Remove" in l
          or "Boss battle only available on the server" in l):
        content.remove(l)
      changed = True
  ...
  while True:
    ...
    for i in range(len(content)):
      if "# TODO BEFORE RELEASING TO PLAYERS" in content[i].upper():
        cmd = ": actual remove block start" if is_fake_boss_file else ": remove block start"
        if cmd in content[i].lower():
          if start is not None:
            ...
            exit(1)
          start = i
        cmd = ": actual remove block end" if is_fake_boss_file else ": remove block end"
        if cmd in content[i].lower():
          if start is None:
            print(
                f"Error redacting {path}: Mismatched TODO block at line {i + 1}"
