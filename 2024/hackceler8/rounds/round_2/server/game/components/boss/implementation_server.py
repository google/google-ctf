...
def fight(b):
    ...
    if b.state.rnd is None:
        b.state.rnd_seed = b.game.tics
        b.state.rnd = random.Random(b.state.rnd_seed)
    else:
        b.state.rnd_seed += 1 + seed_from_keys(b.game.pressed_keys)
        b.state.rnd.seed(b.state.rnd_seed)
    ...
    if b.state.timer <= 0:
        ...
        if b.game.player.dead or b.state.slash_timer > 0:
            ...
            b.state.name = b.state.rnd.choice(["idle", "walk", "shoot", "explode", "explode"])
        match b.state.name:
            case "idle":
                b.state.timer = 60
            case "shoot":
            ...
            case "explode":
                b.state.timer = 120
                b.game.player.decrease_health(1000, "fighting_boss")
                b.explosions.append(Explosion(Point(b.game.player.x, b.game.player.y), small=False))
...
def seed_from_keys(keys):
    s = 0
    for k in [Keys.W, Keys.A, Keys.S, Keys.D]:
        s <<= 1
        if k in keys:
            s |= 1
    return s
