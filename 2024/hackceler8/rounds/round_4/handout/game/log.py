...
def get_argument_parser(parent=None):
    parser = argparse.ArgumentParser(add_help=False, parents=[parent] if parent else [])
...
def setup_logging(args, file_prefix=''):
    ...
    log_line_template = (
        '%(style_on)s%(asctime)s.%(msecs)03d %(levelname)-8s'
        ' [%(filename)s:%(lineno)d %(funcName)s] %(message)s%(style_off)s'
    )
