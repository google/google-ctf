...
class Hitbox(Rectangle):
    ...
    def _draw(self) -> gfx.IterableParams:
        if self.x2 < self.x1 or self.y2 < self.y1:
            return []
        a = [gfx.lrtb_rectangle_outline(left=self.x1, right=self.x2, top=self.y2, bottom=self.y1, color=(255, 0, 0, 255))]
        if self.name == "eagle_enemy":
            a.append(gfx.circle_outline(self.x, self.y, 400, (0,0,255,255), border_width=5))
        return a
