...
class Eagle(Enemy):
    ...
    def __init__(
        ...
        if self.damage is None:
            self.damage = 2
        self.base_walk_data = self.walk_data
        self.speed = 10

    def reset(self):
        if hasattr(self, "base_walk_data"):
            self.walk_data = self.base_walk_data
        super().reset()

    def check_proximity_to_player(self):
        if self.dead or self.game is None or not self._sees_player():
            return

        self.walk_data = WalkData(self, "")

        dx = (self.game.player.x - self.x) * 0.05
        dy = (self.game.player.y - self.y) * 0.05
        d = math.sqrt(dx**2 + dy**2)
        if d > self.speed:
            dx *= self.speed / d
            dy *= self.speed / d

        rect_x_y = self.offset(dx, dy)
        rect_x = self.offset(dx, 0)
        rect_y = self.offset(0, dy)
        for o in self.game.objects + self.game.projectile_system.weapons:
            if o == self or o.nametype == "Element" or o.nametype == "Player":
                continue
            if rect_x_y.collides(o):
                if rect_x.collides(o) and not rect_y.collides(o):
                    dx = 0
                elif not rect_x.collides(o) and rect_y.collides(o):
                    dy = 0
                else:
                    dx = 0
                    dy = 0
    ...
    def tick(self):
        super().tick()
        self.check_proximity_to_player()
