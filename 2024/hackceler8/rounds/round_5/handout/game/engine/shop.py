...
class Shop:
    ...
    def buy(self, item_name):
        ...
        if self.items[item_name].value <= math.ceil(self.current_cash):
    ...
    def buy_bulk(self, item_name, quantity):
        for i in range(quantity):
            if not self.buy(item_name):
                return False
        return True

    def sell_bulk(self, item_name, quantity):
        if self.player_inventory.get(item_name, 0) < quantity:
            return False
        self.current_cash += quantity * self.items[item_name].value
        self.player_inventory[item_name] -= quantity
        return True
