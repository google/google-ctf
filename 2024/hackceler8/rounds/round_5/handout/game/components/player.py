...
class Player(generics.GenericObject):
    ...
    def update_movement(self, pressed_keys, newly_pressed_keys):
        ...
        if self.game.has_item("sticky"):
            if Keys.D in newly_pressed_keys:
                if self.sticky_direction is None:
                    self.sticky_direction = self.DIR_E
                elif self.sticky_direction == self.DIR_E:
                    self.sticky_direction = None
            elif Keys.A in newly_pressed_keys:
                if self.sticky_direction is None:
                    self.sticky_direction = self.DIR_W
                elif self.sticky_direction == self.DIR_W:
                    self.sticky_direction = None
            if self.sticky_direction is not None:
                self.change_direction(self.sticky_direction, sprinting)
        else:
