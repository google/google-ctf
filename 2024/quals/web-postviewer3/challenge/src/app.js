...
const XSSBOT_DOMAIN = process.env.XSSBOT_DOMAIN || "postviewer3-bot";
...
sandbox.use("/:product/", express.static("sandbox"));
...
mainapp.use("/", (req, res, next) => {
  res.set("Content-Security-Policy", `frame-src *.${DOMAIN} blob:`);
  res.set("X-Frame-Options", "DENY");
  next();
});
