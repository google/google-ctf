...
; Implement TEA encrypt: https://en.wikipedia.org/wiki/Tiny_Encryption_Algorithm
...
Encrypt:
    ...
    ; The encryption!!
    ld                  hl,INPUT_BUFFER             ; HL will be out input pointer.
    ld                  de,ENCRYPTED_BUFFER         ; DE will be out output pointer.
    ; Copy all the contents of the input buffer to the encrypted buffer.
    ld                  b,32
.copy_buffer:
    ld                  a,(hl)
    ld                  (de),a
    ...
    ld                  hl,ENCRYPTED_BUFFER         ; DE will be out output pointer.
...
.loop:
    ...
    ; Load the values to the encrypted buffer from V0.
    ld                  ix,V0_RAM
    ld                  b,4
.loadV0:
    ld                  a,(ix)
    ...
    ; Load the values to the encrypted buffer from V1.
    ld                  ix,V1_RAM
    ld                  b,4
.loadV1:
    ld                  a,(ix)
...
.loop:
    ...
    ; -----------------------------------------------
    ; Calculate the first step, this will update v0.
    ; -----------------------------------------------
    call                CalculateV0

    ; -----------------------------------------------
    ; Calculate the second step, this will update v1.
    ; -----------------------------------------------
    call                CalculateV1
