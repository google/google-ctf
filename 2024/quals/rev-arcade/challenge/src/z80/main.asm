...
; Volume for sound, we don't use this.
MVOL							equ $3f

; Channel volume, we don't use this.
CVOL							equ	$c0+$1f

; Sounds, we ended up not using this.
COIN							equ $0000
...
Reset:
	...
	ld				a,0				; Load 0 (again?) to A, I don't remember why I did this since A is already zero.
	
	; Initialize a variable in RAM of 1 in long word format, this will be used by the Encrypt subroutine.
	ld				(ONE),a
	ld				(ONE+1),a
	ld				(ONE+2),a
	ld				a,1
	...
	; Initialize the input count to $30 = $10 (length of key) + $20 (length of flag).
	ld				a,$30
...
.loop:
	...
	;
	; UPDATE: This comment got deprecated, this was for a previous interation of this challenge.
	...
	cp				(ix)				; Compare the contents of IX to the accumulator, in this case 0.
	...
	; $01 -> System ROM calls it when it's about to swap the driver for the cartridge one.
	cp				$01
	jr				z,.prepareSwitch
...
.enableAdminMode:
	...
	; Restore from the stack and return.
	pop				ix
	pop				af
	retn								; Return from NMI.
...
WritePortA:
	...
	; At leaat 6 NOPs to wait for this to go through.
	ld				b,6
.addr_loop:
	nop
	djnz				.addr_loop
...
WritePortB:
	...
	; At least 6 NOPs to wait for this to go through.
	ld				b,6
.addr_loop:
	nop
	djnz				.addr_loop
...
; Play a Coin sound with the sound generator.
PlayCoinSound:
	push				de
...
; All this code below is unused here, but we had it for a former iteration of this challenge.
