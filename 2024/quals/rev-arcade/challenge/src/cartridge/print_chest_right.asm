...
CHEST_RIGHT           equ 535                       ; 
CHEST_RIGHT_OFFSET    equ 24
CHEST_RIGHT_H         equ 4
CHEST_RIGHT_W         equ 4
CHEST_RIGHT_Y         equ $3b6
CHEST_RIGHT_X         equ $c0
...
.scb1_w_loop:                                   ; for (i=0; i<15; i++)
    cmpi.w      #CHEST_RIGHT_W,d0                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_w_done

    ; SCB1 + (i*64)
    move.w      d0,d7
    addi.w      #CHEST_RIGHT_OFFSET,d7
    lsl.w       #6,d7
    addi.w      #SCB1,d7
    ...
    ; tile = START_TILE + i
    move.w      d0,d7
    addi.w      #CHEST_RIGHT,d7
.scb1_h_loop:                                   ; for (j=0; j<9; i++)
    cmpi.w      #CHEST_RIGHT_H,d1                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_h_done
...
.scb1_w_done:
    ...
    move.w      #(SCB2+CHEST_RIGHT_OFFSET),REG_VRAMADDR              ; Target first SCB2.
    nop
    ...
    move.w      #CHEST_RIGHT_Y,d0
    lsl.w       #7,d0
    addi.w      #CHEST_RIGHT_H,d0
    move.w      d0,REG_VRAMRW ; Y position, non-sticky, sprite height in tiles.
    nop

    ; VRAMMOD moved to SCB4 now.
    move.w      #(CHEST_RIGHT_X<<7),REG_VRAMRW               ; X position.
    nop

    move.w       #1,d0                          ; i = 1
.scb2_loop:                                     ; for (i=1; i<15; i++)
    cmpi.w      #CHEST_RIGHT_W,d0                 ; If we reached the end, branch out.
    bge         .scb2_done

    ; *REG_VRAMADDR = (SCB2 + i)
    move.w      d0,d1
    addi.w      #(SCB2+CHEST_RIGHT_OFFSET),d1
