...
.scb1_w_loop:                                   ; for (i=0; i<15; i++)
    cmpi.w      #ROBOT_W,d0                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_w_done

    ; SCB1 + (i*64)
    move.w      d0,d7
    addi.w      #ROBOT_OFFSET,d7
    lsl.w       #6,d7
    addi.w      #SCB1,d7
...
.scb1_h_loop:                                   ; for (j=0; j<9; i++)
    cmpi.w      #ROBOT_H,d1                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_h_done

    move.w      d7,REG_VRAMRW                   ; Draw the tile to VRAM.
    nop

    move.w      #PALETTE6_NOMIRROR,REG_VRAMRW   ; Attributes: Palette 3, no mirror, no animation.
    nop
...
.scb1_w_done:
    ...
    move.w      #(SCB2+ROBOT_OFFSET),REG_VRAMADDR              ; Target first SCB2.
    nop
    ...
    move.w      #$90,X_RAM
    move.w      #($90<<7),REG_VRAMRW               ; X position.
    nop

    move.w       #1,d0                          ; i = 1
.scb2_loop:                                     ; for (i=1; i<15; i++)
    cmpi.w      #ROBOT_W,d0                 ; If we reached the end, branch out.
    bge         .scb2_done

    ; *REG_VRAMADDR = (SCB2 + i)
    move.w      d0,d1
    addi.w      #(SCB2+ROBOT_OFFSET),d1
