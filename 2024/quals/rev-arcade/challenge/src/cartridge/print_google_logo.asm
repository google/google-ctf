...
GAME_LOGO           equ 0                       ; The Google logo is the first sprite on the bank.
GAME_LOGO_H         equ 9
GAME_LOGO_W         equ 15
PALETTE1_NOMIRROR   equ $100
...
.scb1_w_loop:                                   ; for (i=0; i<15; i++)
    cmpi.w      #GAME_LOGO_W,d0                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_w_done
...
.scb1_h_loop:                                   ; for (j=0; j<9; i++)
    cmpi.w      #GAME_LOGO_H,d1                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_h_done

    move.w      d7,REG_VRAMRW                   ; Draw the tile to VRAM.
    nop

    move.w      #PALETTE1_NOMIRROR,REG_VRAMRW   ; Attributes: Palette 1, no mirror, no animation.
    nop

    addq.w      #1,d1                           ; j++
    ; tile += width
    addi.w      #GAME_LOGO_W,d7
    bra         .scb1_h_loop
...
.scb1_w_done:
    ...
    move.w      #($e400+GAME_LOGO_H),REG_VRAMRW ; Y position, non-sticky, sprite height in tiles.
    nop
...
.scb2_loop:                                     ; for (i=1; i<15; i++)
    cmpi.w      #GAME_LOGO_W,d0                 ; If we reached the end, branch out.
    bge         .scb2_done
