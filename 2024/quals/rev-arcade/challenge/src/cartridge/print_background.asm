...
; Print a pixelart background
; The background is 15 tiles wide and 9 tiles tall.

BACKGROUND           equ 135                       ; 
BACKGROUND_H         equ 20
BACKGROUND_W         equ 20
PALETTE4_NOMIRROR    equ $400
...
.scb1_w_loop:                                   ; for (i=0; i<15; i++)
    cmpi.w      #BACKGROUND_W,d0                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_w_done
    ...
    ; tile = START_TILE + i
    move.w      d0,d7
    addi.w      #BACKGROUND,d7
.scb1_h_loop:                                   ; for (j=0; j<9; i++)
    cmpi.w      #BACKGROUND_H,d1                 ; Check if we reached the end, jump out if yes.
    bge         .scb1_h_done

    move.w      d7,REG_VRAMRW                   ; Draw the tile to VRAM.
    nop

    move.w      #PALETTE4_NOMIRROR,REG_VRAMRW   ; Attributes: Palette 3, no mirror, no animation.
    nop

    addq.w      #1,d1                           ; j++
    ; tile += width
    addi.w      #BACKGROUND_W,d7
    bra         .scb1_h_loop
