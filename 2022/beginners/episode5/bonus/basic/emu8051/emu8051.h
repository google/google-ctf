...
class emu8051 {
 public:
  ...
  typedef bool (*xram_mmio_handler)(emu8051 *emu, access_type_t access_type,
                                    uint16_t addr, uint8_t *value,
                                    void *user_data);

  typedef void (*mem_controller)(emu8051 *emu, access_type_t access_type,
                                 uint16_t addr, uint8_t *value,
                                 void *user_data);
  ...
  */
  ...
  // Direct XRAM access, bypasses any MMIO handlers. This is intended to be
  // used by DMA controllers and the like.
  std::array<uint8_t, 0x10000> &xram_get_direct();

  // Set to nullptr to unregister.
  void sfr_register_handler(
        uint32_t addr, sfr_handler handler, void *user_data);
  void xram_register_handler(uint32_t addr, xram_mmio_handler handler);

  // PMEM controller takes over all program memory reads and writes. This allows
  // for banking to be implemented.
  // Set to nullptr to unregister.
  void pmem_register_controller(mem_controller controller, void *user_data);

  // XRAM controller takes over all program memory reads and writes. This allows
  // for banking to be implemented. Note that XRAM handlers have precedence over
  // the XRAM controller.
  // Set to nullptr to unregister.
  void xram_register_controller(mem_controller controller, void *user_data);
 ...
 private:
  ...
  std::array<void*, 0x80> iram_sfr_handlers_user_data_{};

  // External RAM and Program memory.
  std::array<uint8_t, 0x10000> xram_{};
  std::array<uint8_t, 0x10000> pmem_{};
  std::array<xram_mmio_handler, 0x10000> xram_mmio_handlers_{};

  mem_controller pmem_controller_ = nullptr;
  void *pmem_controller_user_data_ = nullptr;

  mem_controller xram_controller_ = nullptr;
  void *xram_controller_user_data_ = nullptr;
