...
CMD kctf_setup && \
    ...
    while true; do \
      if [ "${BLOCK_SUBORIGINS}" = "1" ]; then \
        kctf_drop_privs env MAX_BROWSERS="${MAX_BROWSERS}" BOT_TIMEOUT="${BOT_TIMEOUT}" BLOCK_SUBORIGINS="${BLOCK_SUBORIGINS}" DOMAIN="${DOMAIN}" REGISTERED_DOMAIN="${REGISTERED_DOMAIN}" FLAG="${FLAG}" PAGE_URL="${PAGE_URL}" xvfb-run /usr/bin/node /home/user/bot.js; \
      else \
        kctf_drop_privs env MAX_BROWSERS="${MAX_BROWSERS}" BOT_TIMEOUT="${BOT_TIMEOUT}" BLOCK_SUBORIGINS="${BLOCK_SUBORIGINS}" DOMAIN="${DOMAIN}" REGISTERED_DOMAIN="${REGISTERED_DOMAIN}" FLAG="${FLAG}" PAGE_URL="${PAGE_URL}" /usr/bin/node --trace-warnings /home/user/bot.js; \
      fi; \
    done & \
    kctf_drop_privs \
    socat \
