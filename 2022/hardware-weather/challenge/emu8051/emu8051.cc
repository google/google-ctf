...
emu8051::emu8051() {
  ...
  iram_sfr_handlers_user_data_.fill(nullptr);
...
bool emu8051::verify_address(mem_type_t mem_type,
  ...
  if (addr_type == address_type_t::ADDR_BIT && (
    ...
    fprintf(stderr, "ERROR: Emulator abort at line %i\n", __LINE__);
...
std::array<uint8_t, 0x10000> &emu8051::xram_get_direct() {
  return this->xram_;
}
...
bool emu8051::sfr_write_byte(address_type_t addr_type, uint32_t addr, uint8_t value) {
  ...
  if (addr_type == address_type_t::ADDR_BYTE) {
    ...
    if (handler != nullptr) {
      return handler(
          this, access_type_t::WRITE, address_type_t::ADDR_BYTE,
          addr, &value, iram_sfr_handlers_user_data_.at(index));
    }
  ...
  if (handler != nullptr) {
    return handler(
        this, access_type_t::WRITE, address_type_t::ADDR_BIT,
        addr, &value, iram_sfr_handlers_user_data_.at(index));
  }
...
bool emu8051::sfr_read_byte(address_type_t addr_type, uint32_t addr, uint8_t *value) {
  ...
  if (addr_type == address_type_t::ADDR_BYTE) {
    ...
    if (handler != nullptr) {
      return handler(
          this, access_type_t::READ, address_type_t::ADDR_BYTE, addr, value,
          iram_sfr_handlers_user_data_.at(index)
      );
    }
  ...
  if (handler != nullptr) {
    return handler(
        this, access_type_t::READ, address_type_t::ADDR_BIT, addr, value,
        iram_sfr_handlers_user_data_.at(index)
    );
  }
...
bool emu8051::xram_write_byte(uint32_t addr, uint8_t value) {
  // Note: Handler has precedence over Controller.
  xram_mmio_handler handler = xram_mmio_handlers_.at(addr);
  if (handler) {
    return handler(
        this, access_type_t::WRITE, static_cast<uint16_t>(addr), &value,
        /*USERDATA=*/nullptr
    );
  }
...
bool emu8051::xram_read_byte(uint32_t addr, uint8_t *value) {
  ...
  if (handler) {
    return handler(
        this, access_type_t::READ, static_cast<uint16_t>(addr), value,
        /*USERDATA=*/nullptr
    );
  }
...
bool emu8051::pmem_write_byte(uint32_t addr, uint8_t value) {
  if (pmem_controller_) {
    pmem_controller_(
        this, access_type_t::WRITE, addr, &value, pmem_controller_user_data_
    );
    return true;
  }
...
bool emu8051::pmem_read_byte(uint32_t addr, uint8_t *value) {
  if (pmem_controller_) {
    pmem_controller_(
        this, access_type_t::READ, addr, value, pmem_controller_user_data_
    );
    return true;
  }
...
void emu8051::sfr_register_handler(
    uint32_t addr, sfr_handler handler, void *user_data) {
  if (addr < 0x80 || addr >= 0x100) {
    fprintf(stderr, "ERROR: Emulator abort at line %i\n", __LINE__);
    exit(1);
  }
  ...
  iram_sfr_handlers_user_data_.at(index) = user_data;
}

void emu8051::xram_register_handler(uint32_t addr, xram_mmio_handler handler) {
  if (addr >= 0x10000) {
    fprintf(stderr, "ERROR: Emulator abort at line %i\n", __LINE__);
    exit(1);
  }

  xram_mmio_handlers_.at(addr) = handler;
}

void emu8051::pmem_register_controller(
    mem_controller controller, void *user_data) {
  pmem_controller_ = controller;
  pmem_controller_user_data_ = user_data;
}

void emu8051::xram_register_controller(
    mem_controller controller, void *user_data) {
  xram_controller_ = controller;
  xram_controller_user_data_ = user_data;
}
...
bool emu8051::execute(uint32_t instruction_count) {
  ...
  try {
    ...
    fprintf(stderr, "ERROR: Caught and out_of_range exception: %s\n",
            oor.what());
    exit(1);
  }
...
uint8_t emu8051::sp_get() {
  uint8_t sp;
  sfr_read_byte(address_type_t::ADDR_BYTE, SP, &sp);
  return sp;
}

void emu8051::sp_set(uint8_t value) {
  sfr_write_byte(address_type_t::ADDR_BYTE, SP, value);
}
...
void emu8051::assign_arg(emu8051opcodes::actual_arg_t& arg, uint16_t value) {
  ...
  switch(arg.type) {
    ...
    case arg_t::ARG_A_DPTR_INDIRECT_PMEM:
      fprintf(stderr, "ERROR: Emulator abort at line %i\n", __LINE__);
      exit(1);  // Should never be called (there are no instructions like this).
  }
}
