...
class DanmakuSystem:
    def __init__(self, player, boss, is_server):
        ...
        if not self.is_server:
            ...
            self.gui["warning_title"] = arcade.Text("", 640, 640 - 3,
                                                    ...
                                                    anchor_x="center",
                                                    anchor_y="center")
            self.gui["clock_face"] = arcade.Sprite("resources/villain/clock_face.png", scale=1.5)
            self.gui["clock_face"].alpha = 0
            self.gui["clock_face"].set_position(640, 640)
            self.gui["clock_hand1"] = arcade.Sprite("resources/villain/clock_hand.png", scale=1.5)
            self.gui["clock_hand1"].alpha = 0
            self.gui["clock_hand1"].set_position(640, 640)
            self.gui["clock_hand2"] = arcade.Sprite("resources/villain/clock_hand.png", scale=1.5)
    ...
    def error_fadein(self):
        self.gui["error_message"] = arcade.Text(
                "ERROR: SPATIOTEMPORAL ANOMALY", 640,
    ...
    def tick(self, pressed_keys):
        ...
        if self.game_running:
            if not self.boss.destructing:
                if not self.player.dead and arcade.key.SPACE in pressed_keys and self.ticks % 3 == 0 and self.player.anomaly_speed_multiplier > 0:
