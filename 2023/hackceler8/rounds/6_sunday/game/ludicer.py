...
class Ludicer:
    def __init__(self, net, is_server, debug=False, eager_level_load=True):
        ...
        maze_arena_mapping = {f'{fn[:-4]}_arena': fn[:-4] for fn in
                              map(os.path.basename,
                                  glob.glob('resources/maps/maze_*_map.mgz'))}
        self.arena_mapping = {
            "spike_arena": "spike",
            "logic_arena": "logic",
            "boss_arena": "boss",
    ...
    def get_all_available_items(self):
        ...
        its = [
            ...
            Item(None, name="key_blue", display_name="Blue key", color="blue",
                 wearable=False),
            Item(None, name="magnet", display_name="Magnet",
                 wearable=True),
            Item(None, name="boots", display_name="Boots",
                 wearable=True),
            Item(None, name="noogler", display_name="NooglerHat",
                 wearable=True),
                    ...
                    case "Arena":
                        if o.name in self.arena_mapping:
                            ...
                            if o.stitching:
                                ...
                                match o.dir:
                                    case 'L':
                                        nx = constants.MAZE_REGION_OFFSET + (constants.MAZE_REGION_SIZE - gap_from_boundary)
                                        ny = self.player.prev_y
                                    case 'R':
                                        nx = constants.MAZE_REGION_OFFSET + gap_from_boundary
                                        ny = self.player.prev_y
                                    case 'U':
                                        nx = self.player.prev_x
                                        ny = (next_tiled_map.height_pixel - constants.MAZE_REGION_SIZE) + gap_from_boundary - constants.MAZE_REGION_OFFSET
                                    case 'D':
                                        nx = self.player.prev_x
                                        ny = next_tiled_map.height_pixel - gap_from_boundary - constants.MAZE_REGION_OFFSET

                                player = [o for o in next_tiled_map.objs if o.nametype == 'Player'][0]
                                player.orig_x = nx
                                player.orig_y = ny
