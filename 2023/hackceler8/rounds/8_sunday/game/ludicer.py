...
class Ludicer:
    ...
    def setup_scroller(self):
        ...
        if self.current_map == "danmaku":
            if self.boss_danmaku is None:
                ...
                self.danmaku_system = DanmakuSystem(self.player, self.boss_danmaku,
    ...
    def order_brainduck(self, length=10):
        ...
        order = order[:length]
        logging.info(f"Player collided with Brainduck, entering "
                     f"duck mode. Order: {order}")
    ...
    def handle_duck_logic(self):
        ...
        if duck_input is None:
            ...
            logging.info(f"Total time in maze "
                         f"{time.time() - self.brainduck.start_time}")
            self.brainduck.stop()
