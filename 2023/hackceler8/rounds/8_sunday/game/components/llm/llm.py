...
class Llm():
            ...
            try:
                ...
                if self._about_codeword(response.last):
                    return ("*BEEEEEP*\nCODEWORD-RELATED INFORMATION FOUND IN RESPONSE\n"
                            "CENSORING OUTPUT"), False
                if self._has_letters(response.last.lower(), self.codeword):
                    return "*BEEEEEP*\nCODEWORD TRACES DETECTED IN RESPONSE, CENSORING OUTPUT", False
    ...
    def _about_codeword(self, text):
        text = text.lower()
        return "code" in text or "secret" in text or "destruct" in text or "password" in text
