...
## Files to distribute to player:

The only _required_ files for a player to have is `multiarch` and `crackme.masm`.

The below will let the player fully understand how the runner works and enable them to run a local server.
```
multiarch
crackme.masm
Dockerfile
