...
impl World {
    ...
    const SPAWNED_ENEMY_PROPS: &EnemyProperties = &EnemyProperties {
        walk_data: Self::SPAWNED_ENEMY_WALK_DATA,
        speed: None,
        health: Some(1),
        strength: None,
        invulnerable: false,
    ...
    pub fn update(ctx: &mut Ctx) -> bool {
        if matches!(ctx.world.world_type, WorldType::BossTemple) {
            // Respawn dead minions.
            for (enemy_type, x, y) in [
                (EnemyType::OrcMinion, 40, 88),
                (EnemyType::AngelMinion, 280, 88),
            ] {
                if !ctx.world.enemies.iter().any(|e| e.enemy_type == enemy_type) {
                    let enemy = Enemy::new(
                        enemy_type,
                        x,
                        y,
                        /*id=*/ 1337,
                        Self::SPAWNED_ENEMY_PROPS,
                        &mut ctx.res_state,
                        &mut ctx.vdp,
                    );
                    ctx.world
