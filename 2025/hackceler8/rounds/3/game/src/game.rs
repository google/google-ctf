...
pub struct Game<C: Controllers, R: Renderer, V: Vdp, P: Portal> {
    ...
    pub boss_data: Option<BossData>,
    pub boss_kills_offs: Option<usize>,
}
        ...
        match ctx.state {
            ...
            GameState::MapSwitch => {
                ...
                if let Some(new_world) = ctx.new_world {
                    ...
                    ctx.boss_data = if matches!(new_world, WorldType::BossTemple) {
                        Some(BossData::new(ctx))
                    } else {
                        None
                    };
