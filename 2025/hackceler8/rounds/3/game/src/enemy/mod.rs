...
impl Enemy {
    ...
    pub fn update(ctx: &mut Ctx, enemy_id: usize) {
        ...
        let mut hits = None;
        let mut record_minion_kill = false;
        ...
        match enemy.status {
            Status::Dying {
                ...
                if cooldown == 0
                    && [EnemyType::OrcMinion, EnemyType::AngelMinion].contains(&enemy.enemy_type)
                {
                    record_minion_kill = true;
                }
                cooldown = cooldown + 1;
        ...
        if record_minion_kill {
            let last_hit_player = enemy.last_hit_player;
            let enemy_type = enemy.enemy_type;
            boss::record_minion_kill(ctx, last_hit_player, enemy_type);
        }
        if should_save {
