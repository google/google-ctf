...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sprite_list = [
        ...
        (
            resources_root().join("sprites/items/cloak.tsx"),
            &GamePalette::Player.global(),
        ),
        (
        ...
        (
            resources_root().join("sprites/computer-npc.tsx"),
            &GamePalette::Enemy.for_world("boss-temple"),
        ),
    ];
    ...
    let mut paint_data: Vec<u8> = Vec::new();
    for i in 0..0x60000u32 {
        let b = if (i / 32) % 2 == 0 {
            0x11
        } else {
            let val = (i / 64) % 14 + 2;
        ...
        paint_data.push(b as u8);
    }

    // Add secrets.
    let secret: Vec<u8> = std::fs::read(resources_root().join("secret_tiles.bin"))?;
    assert_eq!(0x200, secret.len());
    let mut addr = 0xfdf00;
    for _ in 0..22 {
        addr += 0x4440;
        for j in 0..0x200 {
            paint_data[addr + j - 0x100000] = secret[j];
        }
    }
    std::fs::write(output_dir().join("paintdata"), &paint_data)?;
