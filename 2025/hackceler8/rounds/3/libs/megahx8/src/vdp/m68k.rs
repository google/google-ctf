...
impl Vdp {
    ...
    */
    ...
    fn set_tiles_raw(&mut self, start_index: u16, src_addr: u32, length: u32) {
        let sz = (start_index as u32) * 32 + length;
        assert!(sz < 0xA000, "VRAM overflow");

        assert_eq!(src_addr % 2, 0, "src misaligned");
        assert!(
            (src_addr >> 1) <= 0x7F_FFFF,
            "Source address unreachable for DMA"
        );

        let dst_addr = start_index << 5;
        self.set_increment(2);
        self.enable_dma(true);

        let dma_length_words = length / 2;
        self.set_register(registers::DMA_LEN_L, dma_length_words as u8);
        self.set_register(registers::DMA_LEN_H, (dma_length_words >> 8) as u8);
        self.set_register(registers::DMA_SRC_L, (src_addr >> 1) as u8);
        self.set_register(registers::DMA_SRC_M, (src_addr >> 9) as u8);
        self.set_register(registers::DMA_SRC_H, (src_addr >> 17) as u8);
        self.set_addr_raw(AddrKind::VRam, dst_addr, /*dma=*/ true);
        self.wait_for_dma();
...
impl super::Vdp for Vdp {
    ...
    fn set_tiles_raw(&mut self, start_idx: u16, start_address: u32, length: u32) {
        self.set_tiles_raw(start_idx, start_address, length);
    }
