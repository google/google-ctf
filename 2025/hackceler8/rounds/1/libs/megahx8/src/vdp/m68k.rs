...
impl Vdp {
    ...
    fn read_into_word_slice(&mut self, kind: AddrKind, src_addr: u16, dst: &mut [u16]) {
        self.set_increment(2);
        self.set_address(kind, src_addr);
        for i in 0..dst.len() {
            dst[i] = unsafe { read_volatile(REG_VDP_DATA16) };
        }
    }
...
impl super::Vdp for Vdp {
    ...
    fn dma_upload_word_slice<T>(&mut self, dst_addr: u16, src: &[T]) {
        self.dma_upload_word_slice(AddrKind::VRam, dst_addr, src);
    }

    fn read_into_word_slice(&mut self, src_addr: u16, dst: &mut [u16]) {
        self.read_into_word_slice(AddrKind::VRamRead, src_addr, dst);
    }
