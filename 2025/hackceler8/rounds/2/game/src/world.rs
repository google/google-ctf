...
impl World {
    ...
    pub fn new(
        ...
        player_position: Option<[Option<(i16, i16)>; 4]>,
    ) -> Self {
    ...
    fn load_map(
        ...
        if self.map.is_none() {
            ...
            if player_position.is_some() {
                for (i, pos) in player_position.unwrap().iter().enumerate() {
                    if let Some((x, y)) = pos {
            ...
            } else if map.player_spawn_position.is_some() {
    ...
    pub fn maybe_scroll_maps(ctx: &mut Ctx) -> bool {
        for player in &ctx.players {
            ...
            if player.y > SCROLL_DOWN_Y && player.y <= SCROLL_DOWN_Y + 4 {
            ...
            if player.y < SCROLL_UP_Y && player.y > SCROLL_UP_Y - 4 {
