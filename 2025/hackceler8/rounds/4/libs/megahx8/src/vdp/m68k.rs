...
impl Vdp {
    ...
    /// Read data from VRAM at the current address.
    ...
    /// Set a color in a given palette.
    fn set_color(&mut self, palette: Palette, pos: u16, color: u16) {
        self.dma_upload_word_slice(
            AddrKind::CRam,
            ((palette as u16) << 5) + (pos << 1),
            &[color],
        );
    }
    /// Get a color in a given palette
    fn get_color(&mut self, palette: Palette, pos: u16) -> u16 {
        self.set_address(AddrKind::CRamRead, ((palette as u16) << 5) + (pos << 1));
        self.read_data()
    }
...
impl super::Vdp for Vdp {
    ...
    /// Get plane tile flags at the given index.
    fn get_plane_tiles(&mut self, plane: super::Plane, dst: &mut [TileFlags]) {
        let base = match plane {
            super::Plane::A => self.plane_a_base,
            super::Plane::B => self.plane_b_base,
            super::Plane::Window => self.window_base,
        ...
        self.set_increment(2);
        self.set_address(AddrKind::VRamRead, base);
        for i in 0..dst.len() {
            dst[i] = TileFlags(self.read_data());
        }
    }
    ...
    fn set_color(&mut self, palette: Palette, pos: u16, color: u16) {
        self.set_color(palette, pos, color);
    }

    fn get_color(&mut self, palette: Palette, pos: u16) -> u16 {
        self.get_color(palette, pos)
    }
