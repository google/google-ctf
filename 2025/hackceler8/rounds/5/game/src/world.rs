...
impl World {
    ...
    fn load_map(
        ...
        if let Some(module_name) = crate::res::maps::map_module_name(self.world_type, x, y) {
            if module_name == "e22" && !self.items_collected.is_set(5) {
                let item = Item::new(
                    ItemType::InvisibleKey,
                    230,
                    150,
                    &ItemProperties { id: 5 },
                    res_state,
            ...
            if module_name == "e20" && !self.items_collected.is_set(8) {
                let item = Item::new(
                    ItemType::InvisibleKey,
                    280,
                    190,
