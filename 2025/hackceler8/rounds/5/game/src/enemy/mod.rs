...
impl Enemy {
    ...
    pub fn update(ctx: &mut Ctx, enemy_id: usize) {
        ...
        let mut hits = None;
        let mut start_boss_dialogue = false;
                ...
                for p in 0..ctx.players.len() {
                    ...
                    if let Some(input) = ctx.controller.controller_state(p) {
                        if player.is_active()
                            ...
                            && player.hitbox().expand(5).collides(&enemy.hitbox())
                        {
                            start_boss_dialogue = true;
                        }
                    }
    ...
    pub fn kill(&mut self, falling: bool) {
        ...
        info!(
            "killed enemy {:x} at {:x}",
            self as *const Enemy as u32,
            Self::kill as *const () as u32
        );
    }
