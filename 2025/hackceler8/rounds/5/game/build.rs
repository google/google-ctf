...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sprite_list = [
        ...
        (
            resources_root().join("sprites/items/invisible_key.tsx"),
            &GamePalette::Player.global(),
        ),
        (
    ...
    for filename in [
        "mod.rs", "w00.rs", "w01.rs", "w02.rs", "w03.rs", "w04.rs", "w05.rs", "w10.rs", "w11.rs",
        "w12.rs", "w13.rs", "w14.rs", "w15.rs", "w20.rs", "w21.rs", "w22.rs", "w23.rs", "w24.rs",
        "w25.rs",
    ] {
        let src = repository_root_dir()
            .join("game/src/water_temple")
            .join(filename);
        let dst = repository_root_dir()
            .join("game/src/res/maps/water_temple")
            .join(filename);
        std::fs::copy(src, dst)?;
    }
