...
impl Converter {
    pub fn new(output_directory: impl AsRef<Path>) -> Self {
        ...
        let mut enemy_types = BTreeSet::new();
        let mut npc_types = BTreeSet::new();
        // Not present on maps this round.
        enemy_types.insert("Siren".to_string());
        npc_types.insert("DuckNpc".to_string());
        item_types.insert("InvisibleKey".to_string());
        Self {
