...
pub struct World {
    ...
    /// Position of the safe tiles to stand on in w11.tmx, for all 4 rows.
    safe_tiles: Option<[u8; 4]>,
}
...
impl World {
    ...
    pub fn update(ctx: &mut Ctx) -> bool {
        ...
        if let Some(safe_tiles) = ctx.world.safe_tiles {
            for player in &mut ctx.players {
                for i in 0..4 {
                    if player.y > 164 + i * 32 && player.y < 188 + i * 32 {
                        let safe_tile = safe_tiles[i as usize] as i16;
                        if player.x < 170 + safe_tile * 32 || player.x > 195 + safe_tile * 32 {
                            player.kill(false);
                        }
                    }
                }
    ...
    /// Load the death tiles (i.e. the safe tile positions) for w11.tmx.
    pub fn maybe_load_death_tiles(ctx: &mut Ctx) {
        if !matches!(ctx.world.world_type, maps::WorldType::WaterTemple) {
            ctx.world.safe_tiles = None;
            return;
        }
        // Hack: Check for the presence of boots to ascertain we're in w11.tmx
        ...
        let mut prng = Prng::new(ctx.frame as u32);
        let mut safe_tiles = [0u8; 4];
        for i in 0..4 {
            safe_tiles[i] = (prng.get_int() % 7) as u8;
        }
        ctx.world.safe_tiles = Some(safe_tiles);
    }
}
