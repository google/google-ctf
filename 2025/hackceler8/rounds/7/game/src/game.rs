...
pub struct Game<C: Controllers, R: Renderer, V: Vdp, P: Portal> {
    ...
    /// Whether to keep equipped items during the next map switch.
    pub keep_equipped_items: bool,
    pub ui: UI,
...
impl Game<TargetControllers, TargetRenderer, TargetVdp, TargetPortal> {
        ...
        match ctx.state {
            ...
            GameState::MapSwitch => {
                ...
                if let Some(new_world) = ctx.new_world {
                    ...
                    if ctx.keep_equipped_items && !matches!(new_world, WorldType::WaterTemple) {
                        if matches!(ctx.world.world_type, WorldType::Overworld) {
                            ctx.keep_equipped_items = false;
                        }
                        Inventory::clear_unequipped(ctx);
                    } else {
                        Inventory::clear(ctx);
                    }
                    let items = ctx.world.inventory.items.clone();
                    ...
                    ctx.world.inventory.items = items;
                    State::clear_screen(&mut ctx.vdp, &[Plane::B]);
                    ctx.new_world = None;
                    ...
                    World::maybe_load_death_tiles(ctx);
                    ctx.state = GameState::Playing;
    ...
    pub fn map_switch(&mut self, new_world: WorldType) {
