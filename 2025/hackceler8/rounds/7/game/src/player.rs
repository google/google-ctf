...
impl Player {
    ...
    pub fn can_be_targeted(&self) -> bool {
        self.is_active() && !self.is_cloaked
    }
    ...
    pub fn update(ctx: &mut Ctx, player_id: usize) -> Option<(i16, i16)> {
        ...
        if let Some(input) = input {
            if !get_player!(ctx, player_id).active && input.just_pressed(Button::Start) {
                ...
                if new_player_pos.is_none() {
                    let p1 = get_player!(ctx, 0);
                    new_player_pos = Some((p1.prev_x, p1.prev_y));
                }
            }
        }
        ...
        // Friendly fire logic.
        let friendly_fire = matches!(player.status, Status::Attacking { cooldown: 20 });
        let (dx, dy) = match player.facing {
            Direction::Right => (1, 0),
            Direction::Left => (-1, 0),
            Direction::Up => (0, -1),
        ...
        let strength = ctx.players[player_id].strength;
        if friendly_fire {
            for p in 0..ctx.players.len() {
                if p == player_id {
                    continue;
                }
                if attack_hitbox.collides(&ctx.players[p].hitbox()) {
                    ctx.players[p].on_hit((dx * 60, dy * 60), strength);
                }
            }
        }
        None
