...
fn secret() -> Dialogue {
    Dialogue::new_free_text("Can you guess my secret tiles?", Some(secret_response))
}

fn secret_response(ctx: &mut Ctx, response: &str) {
    if SECRET.len() == response.len() && SECRET.chars().zip(response.chars()).all(|(a, b)| a == b) {
        ctx.start_dialogue(Dialogue::new_no_response(
            "That's correct! Here's your reward.\n\nObtained *Key* !",
            Some(get_item),
        ));
    } else {
        ctx.start_dialogue(Dialogue::new_no_response("That's not it.\n:C", None));
    }
}

fn password_check() -> Dialogue {
    Dialogue::new_free_text("Enter the password:", Some(password_check_run))
}

fn password_check_run(ctx: &mut Ctx, response: &str) {
    if response.len() != 16 {
        ctx.start_dialogue(Dialogue::new_no_response("Sorry that's not correct.", None));
        return;
    }

    let mut tiles = [Tile::from([0u8; 32]); 32];
    for i in 0..16 {
        let c = response.chars().nth(i).unwrap() as u32;
        let mut data = c;
        for j in 0..2 {
            for _ in 0..(c & 0xF) {
                data = data.wrapping_mul(134775813).wrapping_add(1);
            }
            for k in 0..32 {
                if data & (1 << k) != 0 {
                    tiles[i * 2 + j].0[k] = 0x11;
                }
            }
        }
    }

    ctx.vdp.set_tiles(ctx.res_state.next_tile_pos, &tiles);
    ctx.vdp.set_tiles(
        ctx.res_state.next_tile_pos + tiles.len() as u16,
        check_data::MASK_TILES,
    );
    ctx.vdp
        .set_sprites(0, &check_data::sprites(ctx.res_state.next_tile_pos));

    ctx.vdp.wait_for_vblank();
    ctx.vdp.wait_for_vblank();
    if ctx.vdp.read_state_raw() & 0x20 == 0 {
        ctx.start_dialogue(Dialogue::new_no_response(
            "That's correct! Here's your reward.\n\nObtained *Key* !",
            Some(get_item),
        ));
...
fn exit_with_items() -> Dialogue {
    Dialogue::new_multiple_choice(
        "Would you like to exit with your equipped items kept on you?",
        &["Yes", "No"],
        Some(exit_with_items_2),
    )
}

fn exit_with_items_2(ctx: &mut Ctx, response: &str) {
    if eq(response, "Yes") {
        ctx.keep_equipped_items = true;
        ctx.map_switch(WorldType::Overworld);
    }
}
