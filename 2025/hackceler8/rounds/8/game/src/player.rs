...
impl Player {
    ...
    pub fn update(ctx: &mut Ctx, player_id: usize) -> Option<(i16, i16)> {
        ...
        let map = get_map!(ctx);
        let mut hits = None;
        let mut enemy_to_bunnicate = None;
            ...
            Status::Idle => {
                ...
                if let Some(input) = input {
                    ...
                    if input.just_pressed(Button::A) {
                        ...
                        for (id, enemy) in enemies.iter_mut().enumerate() {
                            if attack_hitbox.collides(&enemy.hitbox()) {
                                if player.bunnicates {
                                    enemy_to_bunnicate = Some(id);
                                } else {
                ...
                if player.moving || ctx.map_changed {
        ...
        let hitbox = player.hitbox();
        let prev_hitbox = Hitbox {
            x: hitbox.x + player.prev_x - player.x,
            y: hitbox.y + player.prev_y - player.y,
            w: hitbox.w,
            h: hitbox.h,
        };
        for enemy in enemies.iter_mut() {
            ...
            if enemy.stats.sings
                && !singer::within_singing_distance(&enemy_hitbox, &prev_hitbox)
                && singer::within_singing_distance(&enemy_hitbox, &hitbox)
            {
                enemy.status = enemy::Status::Singing;
            }
        }
        ...
        if let Some(enemy_to_bunnicate) = enemy_to_bunnicate {
            Enemy::bunnicate(ctx, enemy_to_bunnicate);
        }
        None
    }
