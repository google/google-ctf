...
use crate::big_sprite::AnimState;
...
impl Enemy {
        ...
        match enemy.status {
            ...
            Status::Idle => {
                for p in 0..ctx.players.len() {
                    ...
                    if (dx != 0 || dy != 0) && !map::off_screen(enemy.x, enemy.y) {
    ...
    // From vdp/m68k.rs
    const TILESET_END_ADDRESS: u16 = 0xA000;

    pub fn bunnicate(ctx: &mut Ctx, enemy_id: usize) {
        let enemy = &mut ctx.world.enemies[enemy_id];
        if enemy.is_miniboss() {
            return;
        }

        // Load the rabbit sprite into VRAM.
        let tiles = tileset::TILESETS[rabbit::TILES_IDX];
        let start_tile = if let Some(tile) = ctx.world.rabbit_tile {
            tile
        } else {
            let start_tile = ctx
                .res_state
                .next_tile_pos
                .min(Self::TILESET_END_ADDRESS / 32 - tiles.len() as u16 - 1);
            ctx.world.rabbit_tile = Some(start_tile);
            ctx.vdp.set_tiles(start_tile, tiles);
            ctx.res_state.next_tile_pos += tiles.len() as u16;
            start_tile
        };
        ctx.world.rabbit_tile = Some(start_tile);

        // Replace current sprite with wabbit.
        enemy.sprite = BigSprite {
            start_tile,
            w: 3,
            h: 3,
            sprites: rabbit::SPRITES,
            anims: rabbit::ANIMS,
            animation_state: AnimState::new(rabbit::ANIMS),
            x: enemy.sprite.x,
            y: enemy.sprite.y,
            flip_v: enemy.sprite.flip_v,
            flip_h: enemy.sprite.flip_h,
        };

        // Make the enemy harmless-ish.
        enemy.stats.melee = false;
        enemy.stats.flies = false;
    }
