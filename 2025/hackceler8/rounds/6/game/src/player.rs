...
impl Player {
    ...
    pub fn update(ctx: &mut Ctx, player_id: usize) -> Option<(i16, i16)> {
        ...
        let mut run_boss_hit_logic = false;
        ...
        match player.status {
            ...
            Status::KnockedBack {
                ...
                if cooldown > 20 {
                    ...
                    // Compute hits to check for tiles that apply the color tinting effect.
                    hits = Some(physics::try_move(player, map, &ctx.world.doors, 0, 0));
                }
        ...
        if player.is_active() {
            if let Some(hits) = &hits {
                ...
                if hits.touches_tile(MapTileAttribute::Heal) && !ctx.world.healing_completed {
                    player.heal(1);
                }
                if hits.touches_tile(MapTileAttribute::COLOR_R) {
                    ctx.tint_rgb = (4, 1, 1)
                }
                if hits.touches_tile(MapTileAttribute::COLOR_G) {
                    ctx.tint_rgb = (1, 4, 1)
                }
                if hits.touches_tile(MapTileAttribute::COLOR_B) {
                    ctx.tint_rgb = (1, 1, 4)
                }
            }
        }
