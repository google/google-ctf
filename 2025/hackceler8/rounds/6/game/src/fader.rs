...
pub fn fade_palettes(
    ...
    for p in palettes {
        ...
        for i in 0..color.len() {
            ...
            // Apply tint and darkening.
            for (tint, offs) in [(tint_rgb.0, 0), (tint_rgb.1, 4), (tint_rgb.2, 8)] {
                let mut comp = (new_color >> offs) & 0xF;
                comp = (comp * tint as u16) >> 1;
                comp = (comp * (32 - darkening)) >> 5;
                comp = comp.min(0xF);
                if i == 0 {
                    new_color |= (if tint > 2 { 0xF } else { 0x0 }) << offs;
                } else {
                    new_color &= !(0xF << offs);
                    new_color |= comp << offs;
                }
