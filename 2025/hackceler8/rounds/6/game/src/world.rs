...
pub struct World {
    ...
    pub enemies: heapless::Vec<Enemy, 32>,
...
impl World {
    ...
    pub fn new(
        ...
        let mut map = maps::map(world_type, x, y).unwrap()();
        if matches!(world_type, maps::WorldType::ForestTemple)
            && world.healing_completed
            && x == 1
            && y == 1
        {
            let gfx_layer = map::Array2d::new(GFX_LAYER_FOREST_TEMPLE_11, map::WIDTH, map::HEIGHT);
            map.set_gfx_layer(gfx_layer);
        }
    ...
    pub fn shuffle_chests(&mut self, portal: &mut TargetPortal) {
        ...
        for npc in &mut self.npcs {
            ...
            npc.set_position(128 + 28 + pos as i16 * 24, npc.y);
        }
    ...
    pub fn update(ctx: &mut Ctx) -> bool {
        ...
        if !ctx.world.healing_completed {
            for (_i, player) in ctx.players.iter_mut().enumerate() {
                if player.is_active() {
                    let hits = ctx
                        .world
                        .map
                        .as_ref()
                        .unwrap()
                        .get_hit_tiles(&player.hitbox());
                    if hits.touches_tile(MapTileAttribute::Heal) {
                        ctx.world.healing_completed = true;
                        info!("healing used!");
                        ctx.world.plane_window = PlaneWindow::new();
                        let gfx_layer =
                            map::Array2d::new(GFX_LAYER_FOREST_TEMPLE_11, map::WIDTH, map::HEIGHT);
                        ctx.world.map.as_mut().unwrap().set_gfx_layer(gfx_layer);
                        ctx.world.map.as_mut().unwrap().load_to_vram(
    ...
    fn scroll_maps(ctx: &mut Ctx, direction: Direction) {
        ...
        let mut next_map = initer();
        if matches!(world.world_type, maps::WorldType::ForestTemple)
            && world.healing_completed
            && new_x == 1
            && new_y == 1
        {
            let gfx_layer = map::Array2d::new(GFX_LAYER_FOREST_TEMPLE_11, map::WIDTH, map::HEIGHT);
            next_map.set_gfx_layer(gfx_layer);
        }
