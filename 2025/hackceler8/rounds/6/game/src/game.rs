...
pub const FLAGS_FOR_BOSS_TEMPLE: u16 = 5;
...
pub struct Game<C: Controllers, R: Renderer, V: Vdp, P: Portal> {
    ...
    shot_arrow: bool,
    /// RGB Color tint to apply. 2 for normal, 1 for reduced, 4 for increased tint.
    pub tint_rgb: (u8, u8, u8),
    /// Darkening effect to apply on top of the tint. 0 for fully bright, 32 for fully dark.
    pub darkening: u16,
}
    ...
    pub fn update(&mut self) {
        ...
        match ctx.state {
            GameState::Playing => {
                ...
                let prev_tint = ctx.tint_rgb;
                let prev_darkening = ctx.darkening;
                ctx.tint_rgb = (2, 2, 2);
                ...
                if ctx.tint_rgb != prev_tint || ctx.darkening != prev_darkening {
                    fader::fade_palettes(
            ...
            GameState::MapSwitch => {
                ...
                if let Some(new_world) = ctx.new_world {
                    ...
                    ctx.state = GameState::Playing;
                    ctx.world.shuffle_chests(&mut ctx.portal);
                }
            }
            GameState::Dialogue => {
        ...
        if !ctx.shot_arrow && matches!(ctx.world.world_type, WorldType::SkyTemple) {
            if ctx
                .world
                .npcs
                .iter()
                .any(|n| n.npc_type == NpcType::SnakeNpc)
                && ctx
                    .players
                    .iter()
                    .all(|p| p.is_active() && p.x >= 190 && p.x <= 210)
            {
                ctx.start_dialogue(Dialogue::new_no_response("WATCH OUT!", None));
                ctx.shot_arrow = true;
                for x in [190, 195, 200, 205, 210] {
                    let arrow = Projectile::new(
                        /*shooter_id=*/ 1337,
                        x,
                        /*y=*/ 128,
                        /*dx=*/ 0,
                        /*dy=*/ 100,
                        /*damage=*/ 1337,
                        crate::res::sprites::arrow::new(
                            &mut ctx.res_state,
                            &mut ctx.vdp,
                            /* keep_loaded= */ false,
                        ),
                    );
                    if ctx.world.projectiles.push(arrow).is_err() {
